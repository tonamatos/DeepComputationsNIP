...
The elements of the  $\Gamma$ is called the \emph{computations} of the structure $(L,\mathcal P,\Gamma)$.

If $\Delta\subseteq\Gamma$, we say that $\Delta\subseteq\Gamma$ is \emph{$R$-confined} if $\gamma|_{L[r_\bullet]}:L[r_\bullet]\to L[r_\bullet]$ for every $r_\bullet\in R$ and $\gamma\in \Delta$. Elements in $\overline{\Delta}\subseteq \mathcal L_{sh}$ are called (real-valued) \emph{deep computations} or \emph{ultracomputations}. 
\end{defn}
...
By the preceding remarks, the Extendability Axiom says that the elements of the semigroup $\Gamma$ are definable.
