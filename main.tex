...
Recall that the \emph{raison d'être} of the class of Baire-1 functions is to have a class that  contains the continuous functions but is closed under pointwise limits, and that ( Fact~\ref{baire}) for perfectly normal $X$, a function  $f$ is in $B_1(X,Y)$ if and only if $f^{-1}[U]$ is an $F_\sigma$ subset of $X$ for every open $U\subseteq Y$. This motivates the following definition:
