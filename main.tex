...
Going back to the model theoretic framework, let $$\mathcal F_\varphi(M):=\{\varphi(M,a):a\in M^{|y|}\}$$ be the family of subsets of $M^{|x|}$ defined by instances of the formula $\varphi$, where $\varphi(M,a)$ is the set of $|x|$-tuples $c$ in $M$ for which $M\models \varphi(c,a)$.

The following theorems completely link the concepts presented in this section.

\begin{thm}[Fundamental theorem of statistical learning]
    A collection of sets $\mathcal{C}$ is PAC-learnable if and only if it has finite VC-dimension.
\end{thm}

\begin{thm}[\cite{laskowski1992vapnik}]
...
This work begins with René Baire's doctoral thesis \cite{Baire1899thesis} in which he studied a class of discontinuous functions that are not so far from being continuous. Recall that given two topological spaces $X$ and $Y$ we say that a function $f:X\rightarrow Y$ is continuous if and only if for all open $U\subseteq Y$, $f^{-1}[U]$ is open in $X$. What is the next best thing after being continuous? The best next thing after being an open set (in the Borel hierarchy of sets) is being $F_\sigma$, i.e., a countable union of closed sets (in $\mathbb{R}$ and any metrizable space it is true that every open set is $F_\sigma$). We denote by $B_1(X,Y)$ the set of all functions $f:X\to Y$ such that for all open $U\subseteq Y$, $f^{-1}[U]$ is an $F_\sigma$ subset of $X$ (that is, a countable union of closed sets); we call these types of functions \emph{Baire class 1 functions}. When $Y=\mathbb{R}$ we simply denote this collection by $B_1(X)$. We endow $B_1(X,Y)$ with the topology of pointwise convergence (the topology inherited from the product topology of $Y^X$). By $C_p(X,Y)$ we denote the set of all continuous functions $f:X\rightarrow Y$ with the topology of pointwise convergence. Similarly, $C_p(X):=C_p(X,\mathbb{R})$. Then, the following easily follows from the definitions:

\begin{fact}
If all open subsets of $X$ are $F_\sigma$ (in particular if $X$ is metrizable), then $C_p(X,Y)\subseteq B_1(X,Y)$.
\end{fact}

The preceding fact says precisely that the space of Baire class 1 functions is a bigger space than that of the continuous functions. It is often strictly bigger. As an example, consider the characteristic function of the interval $[0,1]$, i.e., $f:\mathbb{R}\rightarrow\mathbb{R}$ given by $f(x)=1$ if $0\leq x\leq 1$ and $f(x)=0$ otherwise. It is not difficult to check that $f$ is not continuous, but it is Baire class 1. A natural question that arises is, how do topological properties of $X$ translate to $C_p(X)$ and vice versa? These questions, and in general the study of these spaces, are the concern of $C_p$-theory, an active field of research in general topology which was pioneered by A. V. Arhangel’skiĭ and his students in the 1970's and 1980's. This field has found many exciting applications in model theory and functional analysis. Good recent surveys on the topics include \cite{hamel2023cp} and \cite{tkachuk2011cp}. Some results in this paper come from $C_p$-theory but moreover from the theory of the space $B_1(X)$. The following fact is originally due to R. Baire (\cite{Baire1899thesis}) which is contained in his 1899 doctoral dissertation. A modern simple proof can be found in Section 10 of \cite{Todorcevic_1997_TopicsTop}.

\begin{fact}[Baire, 1899]\label{baire}
...
If we intend to generalize our results from $C_p(X)$ to the bigger space $B_1(X)$, we must first find a similar dichotomy. Either every point of closure of the set of functions will be a Baire class 1 function, or there is a sequence inside the set that behaves in the worst possible way (which in this context, is the IP). The theorem we present next is usually not phrased as a dichotomy but rather as an equivalence (with the NIP instead).

We need to introduce a notion of the NIP that is more general than the one from the introduction. It can be interpreted as a sort of continuous version of the one presented in the preceding section. To understand this translation from logic to real-valued functions, it is helpful do draw a helpful analogy with neural networks. Shelah's original definition relies on first-order formulas which output binary truth values, much like a \emph{perceptron}. In contrast, continuous real values are analogous to a sigmoid activator. To recover the discrete combinatorics, we simply threshold the values of the function. Instead of true or false, we ask if the function falls below or above certain values $a$ and $b$ and thus the gap between these numbers is a margin of error, like the decision boundary in a Support Vector Machine. A family of continuous function has the NIP of, no matter how the thresholds are set, the binary classifications fail to shatter the index set.

\begin{defn}
    A family of continuous functions $A\subseteq \mathbb{R}^X$ has the \emph{Non-Independence Property} (NIP) if and only if for every $\{f_n\}_{n\in\mathbb N}\subseteq A$ and for every $a<b$ there are finite disjoint sets $E,F\subseteq\mathbb{N}$ such that
...
We finally have the tools and context necessary to present one of the biggest accomplishments in the field, revealing that the connection between tameness and compactness are not just analogies of each other, but equivalent in a profound way. It justifies why we treat learnability and compactness as two sides of the same coin.
...
\subsection{Todorčević's Trichotomy and different levels of learnability}
