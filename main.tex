...
For a measure $\mu$ on a$X$, the set of all $\mu$-measurable functions will  denoted by $\mathscr{M}^0(X,\mu)$.


We will need the following result about NIP and universally measurable functions:

\begin{thm}[Bourgain-Fremlin-Ta set lagrand, Theorem 2F in \cite{BFT_1978_PCompactBaire}]\label{BFT-2F}
...
\begin{proof}
    By Theorem \ref{BFT-2F}, it suffices to show that $A$ is relatively countably compact in $\mathscr{M}^0(X,\mu)$ for all Radon probability measure $\mu$ on $X$. Since $A$ is Talagrand $\mu$-stable for any such $\mu$, then $\overline{A}\subseteq\mathscr{M}^0(X,\mu)$. In particular, $A$ is relatively countably compact in $\mathscr{M}^0(X,\mu)$.
\end{proof}
