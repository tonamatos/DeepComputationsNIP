...
The map $N_p(z)$ defined above is known as the \emph{Newton's map}.
...
\begin{proof}
    ...
    $$f(x)=\begin{cases}
        ...
        0, & \text{otherwise}.
    \end{cases}$$
    We first prove that $f^{-1}(1)=\{x\in 2^{\mathbb{N}}:x|_{|w|}\leq_{\mathrm{lex}} w\}$ is an open set. Fix $x_0\in f^{-1}(1)$. Let $t:=x_0|_{|w|}$ and consider the basic open set $[t]=\{x\in 2^{\mathbb{N}}:x|_{|t|}=t\}$. Then, it is not difficult to check that $x_0\in [t]\subseteq f^{-1}(1)$. The same reasoning shows that $f^{-1}(0)$ is open.
\end{proof}
...
Mathematically, deep computations are ultralimits of standard computations.
