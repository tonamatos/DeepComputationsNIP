...
<<<<<<< Updated upstream
\section{Probabilistic versions of NIP}
=======
\section{Probabilistic versions of NIP and universal Monte Carlo computability of deep computations}
>>>>>>> Stashed changes
...
=======
\subsection{A probabilistic version of NIP}
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
The main result in section \ref{S:Classification} is that, whenever we work with countably many features, PAC-learning (or NIP) corresponds to relative compactness in the space of Baire class-1 functions.
=======
...
The following result gives a measure-theoretic characterization of the NIP:
...
This result allows us to formalize the concept of a deep computation being computable except on a set of arbitrarily small measure ``no matter which reasonable way you try to measure things on its domain" (see the remarks following definition~\ref{D:universal measurability}).
...
Condition (2) of Theorem~\label{BFT-2F} shows that to study measure-theoretic versions of NIP, we need only consider compact subsets of $X$.
    ...
    For this reason, for the rest of this subsection, we focus on Radon probability measures rather than general Radon measures.
\end{rem}

<<<<<<< Updated upstream
The following probabilistic characterization of the NIP will lead us to find other connections between topology of function spaces and Monte Carlo computability (see the next section).
