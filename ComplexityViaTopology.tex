...
The following is an important  fact about Polish spaces. For a proof, see~\cite[4.3.24]{Engelking:1989}
...
\end{defn}
A Rosenthal compactum~$K$ is necessarily Hausdorff since it is a topological subspace of the Hausdorff product space $\mathbb{R}^X$ (and $\mathbb{R}$ itself is Hausdorff).
...
In such traditional setting, the space of types of a structure is necessarily compact;
...
It is worth pointing out that if we topologize $L$ as the as the standard subspace $[0,1]$ of $\mathbb{R}$, these maps are no longer continuous, and the axiom fails.
