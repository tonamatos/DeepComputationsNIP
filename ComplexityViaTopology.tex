...
The $P$-th coordinate map (natural projection) $\mathbb{R}^{\mathcal{P}}\to \mathbf{R}: \mathbf{v} = (\mathbf{v}_P)_{\mathit{P}\in \mathcal{P}}\mapsto \mathbf{v}_P$, gives a continuous extension to~$\mathcal{L}$ of the predicate $P(\cdot)$ on~$L$;
abusing notation, the extension will still be denoted~$P(\cdot)$.
...
\begin{rem}\label{rem:poor-mans-ultraprod}
  ...
  at least for the purpose of realizing state types with parameters in~$L$, the type space $(\mathcal{L}, \mathcal{P})$ serves as a “poor man’s” ultraproduct of~$(L, \mathcal{P})$ where each type $\mathbf{v}$ is realized by the very definition of~$\mathcal{L}$.
\end{rem}
