...
Recall that any rational map of the complex plane~$\mathbb{C}$ extends continuously to the extended complex plane $\hat{\mathbb{C}} = \mathbb{C}\cup\{\infty\}$.
The \emph{Riemann sphere} is the unit sphere $S^2\subseteq \mathbb{R}^3$ endowed with the stereographic projection map $\mathrm{StPr}: S^2\to\hat{\mathbb{C}}$, whose \emph{inverse} $\mathrm{StIn}^{-1}: \hat{\mathbb{C}}\rightarrow S^2$ (the “stereographic introjection”, so to speak) is given by $z \mapsto  (P_1(z), P_2(z), P_3(z))$, where $(P_1(\infty), P_2(\infty), P_3(\infty)) = (0,0,1)$ and, for $z\ne\infty$,
  \begin{align*}
    P_1(z) &= \frac{2{\Re}(z)}{|z|^2+1},&
    P_2(z) &= \frac{2{\Im}(z)}{|z|^2+1},&
    P_3(z) &= \frac{|z|^2-1}{|z|^2+1}.
  \end{align*}
The extended complex plane $\hat{\mathbb{C}}$ is thus endowed with the real predicates $P_1, P_2, P_3$ with values in~$[-1,1]$, and metrized by the pullback of the usual Euclidean metric on~$S^2$.
...
(the real root $r_0$, red; the roots $r_+ = r_{+1}$ on the upper- and $r_- = r_{-1}$ on the lower- half plane, green and blue, respectively).
...
At any stage, the coloring looks “fuzzy” (out of focus) because the coloring function is continuous;
...
However, the Fatou set also includes points~$z$ for which $(N_p^{(n)}(z))$
...
\begin{exmp}\label{E: computation of square roots}
    \textbf{Computation of square roots}. 
        ...
        Let $L=\mathbb{R}\setminus\{0\}$.
        Let $\mathcal{P}=(P_1,P_2)$ where $x\mapsto (P_1(x),P_2(x))$ is the stereographic projection into $S^2\ni z\mapsto (P_1(z), P_2(z), P_3(z))\subseteq \mathbb{R}^2$, i.e.,
        $$P_1(x)=\frac{2x}{x^2+1},$$
        $$P_2(x)=\frac{x^2-1}{x^2+1}.$$
        Let $\Gamma=\{N_p^n:n\in\mathbb{N}\}$ where
        $$N_p(x)=\frac{x^2+a}{2x}.$$
        As before, $(L,\mathcal{P},\Gamma)$ is a CCS.
        Note that $\mathcal{L}=S^2$ and that each iterate $N_p^n$ can be continuously extended to the extended real line $\mathbb{R}\cup\{\infty\}$, i.e., $\mathcal{L}$.
        For example,
