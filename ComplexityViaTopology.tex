...
\newcommand{\Lshard}{\mathcal{L}_{\text{sh}}}
...
\begin{proof}
  ...
  An arbitrary $\gamma\in\Delta$ restricts to a map $\gamma\restriction L[\rb^{(v)}]: L[\rb^{(v)}]\to L[\rb^{(v)}]$, so $\Gamma \subseteq \prod_{v\in L}\mathcal{L}[\rb^{(v)}]$.
  The latter space is a product of compact spaces, hence compact, so $\overline{\Gamma}$ is a closed, hence compact subset thereof.
