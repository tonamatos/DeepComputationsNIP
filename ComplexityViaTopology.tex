...
This general question, and the study of these spaces in general, is the concern of $C_p$-theory, an active field of research in general topology pioneered by A.~V.~Arhangel’skiĭ and his students in the 1970s and 1980s~\cite{Arkhangelskii:1992}.
$C_p$-theory has found many applications in model theory and functional analysis.
For a recent survey, see~\cite{tkachuk2011cp}.
 
A function $f: X\to Y$ between topological spaces is said to be of \emph{Baire class~1}, or a \emph{Baire-1} function, if there is a sequence $(f_n)_{n\in \mathbb{N}}$ of continuous functions $f_n:X\to Y$ such that $f(x)=\lim_{n\to\infty}f_n(x)$ for all $x\in X$.
If $X$ and $Y$ are topological spaces, the space of Baire-1 functions $f:X\rightarrow Y$ endowed with the topology of pointwise convergence is denoted $B_1(X,Y)$ (we abbreviate $B_1(X, \mathbb{R})$ as $B_1(X)$);
...
A elementary argument shows that Baire-1 functions are continuous everywhere except on a set of the first category (also called a \emph{meager} set, i.e., any countable union of sets whose closures have empty interior;
intuitively, these sets are ``topologically small'', at least in Baire spaces—see below).
Thus, Baire-1 functions are continuous on a ``topologically large'' subset of their domain.
...
In Baire spaces, a meager subset is “small” in the sense that it includes no nonempty open set.
...
Important state spaces include $L=\mathbb{R}^\mathbb{N}$ and $L=\mathbb{R}^n$ for some positive integer~$n$, endowed with predicate $P_i(v) = v_i$ giving the $i$-th coordinate of~$v$ for each~$i$.
(These are state spaces endowed with either a finite or countable set of predicates.)
...
\begin{rem}
\label{R:types}
(For readers versed in model theory.)
Computation states structures are real-valued structures (see~\cite{Keisler:2023}; see also~\cite[section 6]{Duenez-Iovino:2017} for a tutorial);
they form a class closed under ultraproducts and ultrapowers. 
In particular, given a structure $\underline{L}=(L,\mathcal{P})$, every type in $\mathcal{L}\setminus L$ is realized in some ultrapower~$\underline{L}^{(\mathcal{U})}$ of~$\underline{L}$.
Indeed, if $\mathbf{v}\in\mathcal{L}$ is an arbitrary type, by the well-known of characterization of topological closure in Hausdorff spaces, there exists an indexed  family $(v_i)_{i\in I}$ in~$L$ and an ultrafilter~$\mathcal{U}$ on~$I$ such that $\mathbf{v} = \mathcal{U}\lim_i v_i$;
this means that $\mathbf{v}$ is realized in the $\mathcal{U}$-ultrapower $\underline{L}^{(\mathcal{U})}$ of~$\underline{L}$ (whose universe consists of equivalence classes “modulo~$\mathcal{U}$” of tuples $(v_i)_{i\in I}\in L^{\mathcal{U}}$ which $\mathcal{U}$-converge in type in~$\mathcal{R}^{\mathcal{P}}$, i.e., modulo the relation of having equal $\mathcal{U}$-ultralimit in type).
...
\begin{defn}
  ...
  Given a sizer $r_\bullet$, let $\mathbb{R}^{[\rb]} = \prod_{P\in\mathcal{P}}[-r_P,r_P]$ (a compact space), and let the $r_\bullet$-\emph{shard} of the states space~$L$ be
\[
...
We take $L\subseteq \mathbf{R}^{\mathcal{P}}$ as implicitly identified with a subset of the type space~$\mathcal{L} \subseteq \mathbb{R}^{\mathcal{P}}$ of~$(L, \mathcal{P})$.
...
We topologize $\Gamma$ as a subset of the topological product $L^L$, where the ``exponent''~$L$ serves merely as an index set, but the ``base''~$L\subseteq \mathbb{R}^{\mathcal{P}}$ is topologized as a subspace (as usual).
In this manner, the semigroup $\Gamma$ is identified with a subset of the topological product~$\mathcal{L}^L \subseteq (\mathbb{R}^{\mathcal{P}})^L$, which leads to an inclusion of the closure $\overline{\Gamma}\subseteq \mathcal{L}^{L}$.
