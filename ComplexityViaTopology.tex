...
\subsection{Example: Computations on Cantor’s set}\label{subsec:finite_prec_threshold}
...
Let $\mathcal{P} = (P_n)_{n\in\mathbb{N}}$ be the collection of projections (coordinates) $P_n:x \mapsto x_n$.
%In other words, $P_n$ is the predicate that reads the $n$th digit of an input sequence.
Since $2^{\mathbb{N}} \subseteq \mathbb{R}^{\mathbb{N}}$ ($\cong \mathbb{R}^{\mathcal{P}}$) is closed, the type space of $(2^{\mathbb{N}},\mathcal{P})$ is effectively still $2^{\mathbb{N}}$.
The constant $0$ and $1$ elements of $2^\mathbb{N}$ are denoted $0_{\bullet}$ and $1_{\bullet}$, respectively.
In this way, we obtain the \emph{Cantor computation states structure (CSS)} $\mathcal{C} = (2^{\mathbb{N}},(P_i)_{i\in \mathbb{N}})$.
...
\emph{Prefixing} by a given word $w\in 2^{<\mathbb{N}}$ means applying the operation $\arc{w\cdot}: x\mapsto \arc{wx}$;
when used as the name of the prefixing operation~$\arc{w\cdot}$, we call $w$ a \emph{prefix}.

\subsubsection{Shift action}
\label{sec:shifts}
An altogether different action on $2^{\mathbb{N}}$ is given by shift operations;
namely, a fixed $m\in \mathbb{N}$ acts on $2^{\mathbb{N}}$ by the map
\begin{equation*}
  \ola{m} : x = (x_i)_{i\in \mathbb{N}}\mapsto \ola{m}x
  \coloneq (x_{m+i})_{i\in \mathbb{N}}.
\end{equation*}
Here, $\ola{0}: 2^{\mathbb{N}}\to2^{\mathbb{N}}$ is the identity, and the equality $(\ola{m+n})x = \ola{m}(\ola{n}x)$ for $m,n\in \mathbb{N}$ holds identically (i.e., the additive semigroup (monoid) $(\mathbb{N},0,+)$ is represented as the semigroup of shifts of~$2^{\mathbb{N}}$).
...
For any fixed word~$w\in2^n$, let ${\preccurlyeq}_nw\coloneqq\Brack{\cdot\restriction_n \preceq w}$ be the relation $x\mapsto \Brack{x\restriction_n\preceq w}$ on~$2^{\mathbb{N}}$.
We shall write $x{\preccurlyeq}_nw$ to mean $x\restriction_nw$
(i.e., the relation symbol ${\preccurlyeq_n}$ takes its argument $x$ on the left).
...
%   x&\mapsto
%   \begin{cases}
%     1_{\bullet}, & x\restriction_n \preceq w;\\
...
—it is a threshold classifier with negative precision~$n=-1$, so to speak.

We remark that the statement “$x\preccurlyeq_nw$” has identical semantics to “$x\preceq w1_{\bullet}$” (not to “$x\preceq w0_{\bullet}$”) in the usual ordering semantics of $2^{\mathbb{N}}$ which depend on all entries of~$x$, not just those of $x\restriction_n$.
...
It is easily shown that such accumulation points~$\varphi$, henceforth called \emph{general threshold classifiers}, must have one of the forms%
% \footnote{The identically true relation $\mathbf{1} = {\preccurlyeq}{()}$ is a general classifier.
%   However, the identically false relation $\mathbf{0}$ —although of the form $\widehat{\phi}_{0_{\bullet}}$— is not a general classifier, because all general classifiers $\varphi$ satisfy~$\varphi(0_{\bullet}) = 1$.
%   \label{fn:constant-relations}}
\begin{itemize}
\item $\Brack{\cdot\preceq z}$, denoted ${\preccurlyeq}z$, or
\item $\Brack{\cdot\prec z}$, denoted ${\prec}z$
\end{itemize}
for some sequence $z\in 2^{\mathbb{N}}$.
...
for clarity, one may always write ${\preccurlyeq}_nw$ where $n=|w|$.

Any finite-precision classifier is a general classifier.
Indeed, ${\prec}0_{\bullet}$ is the false relation $\mathbf{0}$ (in agreement with our earlier convention);
other finite-precision classifiers are of the form ${\preccurlyeq}_nw = \Brack{\cdot\preceq\vec{w}}$ (i.e., ${\preceq}\arc{w1_{\bullet}}$) as remarked above.

Since (countably many) sequences of the form $\vec{w}$ for words $w\in 2^{<\mathbb{N}}$ are dense in~$2^{\mathbb{N}}$, one sees that general classifiers are \emph{sequential} limits of finite classifiers~${\preccurlyeq}w$, i.e., Baire-$1$ functions on~$2^{\mathbb{N}}$.

A general threshold classifier not of finite precision is called \emph{of infinite-precision};
such infinite-precision classifiers are \emph{discontinuous} Baire-1 functions on~$2^{\mathbb{N}}$:
the sole point of discontinuity of an infinite-precision classifier ${\preccurlyeq}z$ or ${\prec}z$ is~$z$.

Let $\Delta = \{{\preccurlyeq}w:w\in 2^{<\mathbb{N}}\} \cup \{\mathbf{0}, \mathbf{1}\}$, where $\mathbf{0},\mathbf{1}: 2^{\mathbb{N}}\rightarrow 2^{\mathbb{N}}$ are the constant maps $x\mapsto0_{\bullet}$ and $x\mapsto1_{\bullet}$, respectively. 
...
The crucial difference between Newton’s method and threshold classifiers is that the complexity of the former comes from \emph{depth}:
the semigroup is generated by a single map but its iterates are highly complex;
the complexity of threshold classification comes from \emph{width}:
the semigroup has infinitely many generators, but their compositions are simple.

Intuitively, the closure $\overline{\Delta}$ consists of the set of all possible threshold classifiers on the real line, but there are two sorts:
the ones that classify strict inequalities and those that classify $\leq$.
The members of $\Delta$ are finite-precision approximations of classifiers that check all bits of information.
But here it gets interesting: what is the difference, in terms of exact (i.e., infinite-precision) arithmetic, between $x<0.5$ and $x\leq0.5$?
...
\begin{proof}
    ...
    Specifically, for every $x\in L$, we intend to prove that $\lim_{n\to\infty}{\preccurlyeq}{w_n}(x)=f_a^+(x)$.
    ...
    Crucially, $w_n(m)=0<1=x(m)$, which implies that $w_n<_{\mathrm{lex}}x|_{n+1}$, and hence ${\preccurlyeq}{w_n}(x)=0^{\infty}=f_a^+(x)$ for large enough $n$.
    If $x\leq_{\rm lex}a$, then $x|_{n+1}\leq_{\rm lex}w_n$ for all $n\in\mathbb{N}$.
    Hence, ${\preccurlyeq}{w_n}(x)=1^\infty=f_a^+(x)$ for all $n\in\mathbb{N}$.
    ...
    \begin{enumerate}
        ...
        We claim that $\lim_{n\to\infty}{\preccurlyeq}{w_n}(x)=f_a^-(x)$.
        Assume that $x<_{\rm lex}a$.
        Then, $x|_{|w_n|}\leq_{\rm lex}w_n$ for large enough $n$.
        Hence, ${\preccurlyeq}{w_n}(x)=1^\infty=f_a^-(x)$ for large enough $n$.
        Now assume that $x\geq_{\rm lex}a$.
        Then, $w_n<_{\rm lex} a|_{|w_n|}\leq_{\rm lex} x|_{|w_n|}$ so ${\preccurlyeq}{w_n}(x)=0^\infty=f_a^-(x)$ for all $n\in\mathbb{N}$.
...
Indeed, $\overline{P_n\circ \Delta}$ contains the \emph{Split Cantor} space for all $n\in\mathbb{N}$. (see Example~\ref{exa:Rosenthal-compacta}-(3)).

\subsection{Example: Finite precision prefix test}
...
In fact, $\overline{\Delta}=\Delta\cup\{\delta_a:a\in 2^\mathbb{N}\}\cup\{\mathbf{0^\infty}\}$ and this space is known as the \emph{Extended Alexandroff compactification of $2^\mathbb{N}$} (see Example \ref{exa:Rosenthal-compacta}-(2)).
...
This space was obtained as the closure of the space discussed in Section~\ref{subsec:finite_prec_threshold}, thus giving an example that separates $\NIP_2$ from $\NIP_3$.
