...
We identify new complexity classes by using topology of function spaces, specifically, the classification of Rosenthal compacta.
The language of model theory, specifically, the concept of \emph{independence} from Shelah's classification theory, gives us a medium to translate between topology and computation.
...
We apply a result of Bourgain, Fremlin and Talagrand from the late 70s on compact subsets of the first Baire class to obtain a new ``tame vs wild'' dichotomy for  deep computations~\cite{BFT_1978_PCompactBaire}; 
then, we apply a trichotomy of Todorčević, from the late 90s, for \emph{Rosental compacta} to identify new tame classes~\cite{Todorcevic_1999_CompactSubsetsBaire}.
...
At the time of writing this paper, decades after his original argument, no elementary proof has been found; 
in fact, the proof method has been analyzed and exploited further~\cite{Todorcevic:2023,horowitz2019compactsetsbaireclass}.

Through our Rosetta stone, Rosenthal compacta in topology correspond to the important concept of  \emph{Non Independence Property} (known as ``NIP'') in model theory, identified by Shelah~\cite{Shelah:1971,Shelah:1990}, and to the concepts \emph{VC-dimension} and \emph{Probably Approximately Correct} learning (known as ``PAC learnability'') in statistical learning theory~\cite{Vapnik-Chervonenkis:1971, Vapnik-Chervonenkis:1974, Valiant:1984}.
...
Fom the point of view of approximation theory, it is noteworthy that in the context of Grothendieck~\cite{Grothendieck:1952} and  Bourgain-Fremlin-Talagrand~\cite{BFT_1978_PCompactBaire}, compactness and sequential compactness coincide; thus, the results of these papers allow us to ensure that deep computations, which in principle are abstract ultrafilter limits (over a potentially uncountable set), can always be approximated by concrete  \emph{sequences} of standard computations.
...
Recall that the product topology on these spaces is the topology of pointwise convergence: a sequence converges in the space if and only if it converges at each coordinate index.
...
In symbols, $f:X\to Y$ is \emph{Baire class~1} if there is a sequence of continuous functions $f_n:X\to Y$ such that for all $x\in X$, $f(x)=\lim_{n\to\infty}f_n(x)$.
...
Clearly, $C_p(X,Y)\subseteq B_1(X,Y)\subseteq Y^X$ and we give these the topology (called the \emph{topology of pointwise convergence}) inherited from the product topology of $Y^X$.
...
An elementary fact about Baire class~1 functions is that they are continuous everywhere except on a set of first category (also called a \emph{meager} set, which is a countable union of sets whose closures have empty interior; intuitively, these sets are ``topologically small'').
Thus, Baire class~1 functions are continuous on a ``topologically large'' subset of their domain.
...
Relatively compact subsets of $B_1(X)$ (for $X$ Polish) are of interest in analysis and topological dynamics.
...
The following are equivalent:
    \begin{enumerate}[(i)]
        ...
        \item
        $A$ is relatively countably compact in $B_1(X)$, i.e., every countable subset of $A$ has a limit point in $B_1(X)$.
        \item
...
In metrizable spaces, points in the closure of some subset can always be approximated by a convergent sequence of points in the set.
For more complicated spaces, such as $C_p$-spaces, this fails in remarkable ways.
The $n$-th coordinate map $p_n: 2^{\mathbb{N}}\to\{0,1\}$ on the Cantor space $X = 2^\mathbb{N}$ ($= \{0, 1\}^{\mathbb{N}}$) is continuous for each $n\in \mathbb{N}$, and one can show (e.g., \cite[Chapter~1.1]{Todorcevic_1997_TopicsTop}) that $\{p_n\}_{n\in\mathbb{N}}$ has \emph{no} convergent subsequences, in $\mathbb{R}^X$.
In a sense, this example exhibits the worst failure of sequential convergence possible.
The closure of $\{p_n\}$ in $\{0, 1\}^X$ (or in $\mathbb{R}^{X}$ for that matter) is homeomorphic to the \emph{Stone-Čech compactification} of the discrete space of natural numbers, usually denoted $\beta\mathbb{N}$, which is an important object of study in general topology.
...
Rosenthal's Dichotomy states that a pointwise bounded set of continuous functions contains either a convergent subsequence, or a subsequence whose closure is essentially the same as the example mentioned in the previous paragraphs (i.e., the ``wildest'' possible). The genesis of this theorem was Rosenthal's ``$\ell_1$-Theorem'', which states that a Banach space includes an isomorphic copy of $\ell_1$ (the space of absolutely summable sequences), or else every bounded sequence therein is weakly Cauchy.
...
This result is usually not phrased as a dichotomy, but rather as an equivalence (see Theorem~\ref{BFT} below).
...
\begin{proof}
  ...
  Fix a sequence $\{f_n\}\subseteq A$ and reals $a<b$ such that $X^{f_E}_{\le a}\cap X^{f_F}_{\ge b}\neq\empty$ for every finite disjoint $E,F\subseteq\mathbb{N}$.
  Fix $I\subseteq\mathbb{N}$.
  Since $\{f_n\}\subseteq A\subseteq C_p(X)$ is a sequence of continuous functions, the family $\mathcal{F}_I=\{X^{f_{\{n\}}}_{\le a}:n\in I\}\cup\{X^{f_{\{m\}}}_{\ge a}:m\in I^{\complement}\}$ consists of closed (non-empty) sets.
  Since all finite $E\subseteq I$ and $F\subseteq I^{\complement}$ witness finitary IP for $\{f_n\}$ and $a, b$, then the family $\mathcal{F}_I$ has the finite intersection property.
...
A structural approach to arbitrary-precision arithmetic}
\label{S:CCS}

In this section, we connect function spaces with arbitrary-precision arithmetic computations.
...
Important state spaces are $L=\mathbb{R}^\mathbb{N}$ and $L=\mathbb{R}^n$ for some positive integer $n$, endowed with predicate $P_i(v) = v_i$, one each for the $i$-th coordinate of~$v$.
...
\begin{rem}
\label{R:types}
This remark is for readers versed in model theory. Computation states structures are real-valued structures (see~\cite{Keisler:2023}; see also~\cite[section 6]{Duenez-Iovino:2017} for a tutorial), and the class of computation state structures is closed under ultraproducts and ultrapowers. 
Given a structure $\mathbb{L}=(L,\mathcal{P})$, every type in  $\mathcal{L}\setminus L$ is realized in an ultrapower of $\underline{L}$, for if $f\in\mathcal{L}\setminus L$, then, by the definition of topological closure, there exists an indexed  family $(f_i)_{i\in I}$ in $L$ and an ultrafilter $\mathcal{U}$ on $I$ such that $f=\mathcal{U}\lim_i f_i$; this means that $f$ is realized in the $\mathcal{U}$-ultrapower  $\mathbb{L}_\mathcal{U}$ of $\mathbb{L}$; 
in fact, $f$ is realized by the element of $\mathbb{L}_\mathcal{U}$ represented by $(f_i)_{i\in I}$. 
...
As we combine ideas of model theory~\cite{Keisler:2023} and topology~\cite{BFT_1978_PCompactBaire}, we are interested in families of real-valued functions that are pointwise bounded. This leads us to the concepts of \emph{sizer} and \emph{shard} first introduced in~\cite{alva2024approximability}:
...
In general, $\mathcal{L}_{\text{sh}}\subseteq \mathcal{L}$, and the inclusion may be proper.
However, equality holds in the important special case when $\mathcal{P}$ is countable (see~\cite{alva2024approximability}).
...
\begin{rem}
The remark is related to remark~\ref{R:types}. 
Compositional computational structures as defined above are not real-valued structures in the sense of~\cite{Keisler:2023}.
However, they can be easily defined alternatively as real-valued structures as follows. We define a CCS as a quintuple $(L,\mathcal{P},\Gamma,\circ,\operatorname{ev})$, where
\begin{itemize}
\item
...
\item 
$\operatorname{ev}: \Gamma\times L\to L: (\gamma,v)\mapsto \operatorname{ev}(\gamma,v)$ is a map giving an action of $\Gamma$ on~$L$
  (i.e., $\operatorname{ev}(\gamma\delta,v) = \operatorname{ev}(\gamma,\operatorname{ev}(\delta,v))$ for $\gamma,\delta\in\Gamma$ and $v\in L$).
\end{itemize}
Then, notationally, we identify each $\gamma\in \Gamma$ with the map  $(\gamma,v)\mapsto \operatorname{ev}(\gamma,v)$.
This is in fact the original definition of CSS given in \cite{alva2024approximability}.
...
Let  $f: L\to \mathcal{L}$ be a function that maps each input state type  $(P(v))_{P\in \mathcal{P}}\in\mathbb{R}^\mathcal{P}$ to an output state type $(P\circ f (v))_{P\in \mathcal{P}}\in\mathbb{R}^\mathcal{P}$.
 ...
 \begin{enumerate}
 \item
We will say that $f$ is (first-order) \emph{definable} if for each $Q\in\mathcal{P}$, the output feature $Q{\circ}f: L\to\mathbb{R}$ is a definable predicate in the following sense: 
For every $\epsilon>0$ and every compact $K\subseteq L$,  there 
is an \emph{approximating function} $\varphi_{Q,\,K,\varepsilon}: L\to \mathbb{R}$ that can be built recursively out of a finite number of the (primitively computable) predicates in $\mathcal{P}$
by a finite number of iterations of the finitary lattice operations $\land$ (=min) and $\lor$ (=max), the operations of $\mathbb{R}^\mathbb{R}$ as a vector algebra (that is, vector addition and multiplication and scalar multiplication) and the operators sup and inf applied on individual variables from $L$, and such that 
\[
...
\emph{Remark:} What we have defined above is a model-theoretic concept; it is a special case of the concept of \emph{first-order definability} for real-valued predicates in the model theory of real-valued structures first introduced in \cite{Iovino:1994} for model theory of functional analysis and now standard in model theory (see~\cite{Keisler:2023}).
...
\item
We will say that $f$ is $\mathcal{P}$-\emph{computable} if it is definable in the sense defined above under~(1), but without the use of the sup/inf operators; in other words, if for every choice of $Q,K,\varepsilon$, the approximation function $\varphi_{Q,K,\varepsilon}: L\to \mathbb{R}$ can be constructed without any use of sup or inf operators; in other words,
 $\varphi_{Q,K,\varepsilon}$ can be built recursively out of a finite number of the (primitively computable) predicates in $\mathcal{P}$
by a finite number of iterations of the (primitively computable) finitary lattice operations $\land,\lor$, and the  (primitively computable) operations of $\mathbb{R}^\mathbb{R}$ as a vector algebra.

\emph{Remark:} This is the model-theoretic concept of quantifier-free definability (i.e., definability as given by the preceding paragraph, but without use of quantifiers).
 \end{enumerate}
...
To summarize, for a function a function $f: L\to \mathcal{L}$, the following conditios are equivalent:

\begin{itemize}
\item
$f$ is $\mathcal{P}$-computable
\item
$f$ is definable
\item
$f$
$f$  is polynomially approximable 
\item
$f$ can be extended to a continuous $\tilde{f}: \mathcal{L} \to \mathcal{L}$.
\end{itemize}
...
\begin{itemize}
    \item $L_p$ is the set of all $z\in\mathbb{C}$ such that there exists an open neighborhood $U$ of $z$ such that every sequence in $\{N_p^n:n\in\mathbb{N}\}$ has a subsequence that converges uniformly on compact subsets of $U$, where
    $$N_p(z)=z-\frac{p(z)}{p'(z)}$$
    and $N_p^n:=N_p\circ N_p \circ \cdots\circ N_p$ is the nth-iteration of $N_p$.
    \item $\mathcal{P}:=\{P_1,P_2,P_3\}$ where
    $$P_1(z)=\frac{2{\rm Re}(z)}{|z|^2+1},$$
    $$P_2(z)=\frac{2{\rm Im}(z)}{|z|^2+1},$$
...
\begin{rems}\hfill
    \begin{enumerate}
        \item The set $L_p$ is known as the \emph{Fatou set} of $N_p$ (see section 2 in \cite{Blanchard1984}).
        Its complement $\mathbb{C}\backslash L_p$, is known as the \emph{Julia set} of $N_p$.
        \item The map $z\mapsto (P_1(z),P_2(z),P_3(z))$ is the stereographic projection into the Riemann Sphere $S^2$.
        \item The set $L_p$ is open and dense in $\mathbb{C}$ (\cite{Blanchard1984}, Corollary 4.6). Hence, its closure $\mathcal{L}$ in $\mathbb{R}^\mathcal{P}$ is the Riemann sphere $S^2$ (i.e., the extended complex plane).
        \item The set $L_p$ is completely invariant under iterations of $N_p$, i.e., $N_p(L_p)=L_p$ and $N_p^{-1}(L_p)=L_p$.
        This implies that all iterations $N_p^n:L_p\rightarrow L_p$ are transition maps.
...
The map $N_p(z)$ defined above is known as \emph{Newton's map}.
...
For each root $r\in\mathbb{C}$ of $p(z)$, there exists an $\varepsilon>0$ such that for any initial guess $z_0$ in the $\varepsilon$-ball centered at $r$, Newton's iteration converges to $r$ (provided $p'(r)\neq 0$) and the convergence is quadratic in that case, meaning the error at each step is roughly squared, causing the number of correct digits to double, leading to fast convergence. 
...
The set of such points, i.e. the \emph{Julia set} associated to $N_p$, is typically a fractal.
This can be visualized by adding a dash of color:
...
At each stage, the complex plane looks as if out of focus because the coloring function is continuous.
As the reader can see in Figure~\ref{fig:newton}, the points at the boundary of each color class form the famous Newton's fractal (of which, interestingly, Newton was unaware).
...
In this case, there are three basins of attraction (one for each root) and the complement of their union is the Julia set, i.e., the common boundary.
...
\begin{prop}
    ...
    $(L_p,\mathcal{P},\Gamma_p)$ satisfies the Extendibility Axiom.
\end{prop}
...
\begin{exmp}\label{E: computation of square roots}
    \textbf{Computation of square roots}. 
        ...
        Let $L=\mathbb{R}\backslash\{0\}$.
        Let $\mathcal{P}=(P_1,P_2)$ where $x\mapsto (P_1(x),P_2(x))$ is the stereographic projection into $S^1\subseteq \mathbb{R}^2$, i.e.,
        $$P_1(x)=\frac{2x}{x^2+1},$$
        $$P_2(x)=\frac{x^2-1}{x^2+1}.$$
        Let $\Gamma=\{N_p^n:n\in\mathbb{N}\}$ where
        $$N_p(x)=\frac{x^2+a}{2x}.$$
        As before, $(L,\mathcal{P},\Gamma)$ is a CCS.
        ...
        For example,
        $$\tilde{N_p}(x)=\begin{cases}
            \frac{x^2+a}{2x}, & \text{if }x\in L;\\
            \infty, & \text{if }x=0,\infty.
        \end{cases}$$
        ...
        The set $\overline{\tilde{\Gamma}}$ of deep computations is $\tilde{\Gamma}\cup\{\tilde{f}\}\subseteq B_1(\mathcal{L},\mathcal{L})$.
\end{exmp}
...
\begin{exmp}
    \textbf{Newton's method for $p(z)=z^3-2z+2$}.
        ...
        Then, $L_p=B\cup\bigcup_{i=1}^3 B_i$.
        ...
        Note that for $z\notin L_p$, no subsequence of $\tilde{N_p^n}(z)$ converges to a complex number.
        However, since $\mathcal{L}=S^2$ is compact there is a subsequence of $\tilde{N_p^n}(z)$ that converges to $\infty$.
        We can extend $f_i:L_p\rightarrow\mathcal{L}$ to $\tilde{f_i}:\mathcal{L}\rightarrow\mathcal{L}$ by:
        $$\tilde{f_i}(z)=\begin{cases}
           f_i(z), & \text{if }z\in L_p;\\
           \infty, & \text{if }z\notin L_p.
        \end{cases}.$$
        Again, note that $\tilde{f_i}$ for $i=1,2$ are not continuous and that $\tilde{f_i}\in\overline{\tilde{\Gamma}}$.
\end{exmp}
...
Given a finite binary string $w$, we consider the transition $\phi_w:L\to L$ given by the rule $$\phi_w(x)=\begin{cases}1^\infty, & \text{if }x|_{|w|}\leq_{\mathrm{lex}}w;\\0^\infty, & \text{otherwise},\end{cases}$$ 
...
Since the sets $\{x\in 2^\mathbb{N}:x|_{|w|}=w\}$ are open and closed in $2^\mathbb{N}$, then the transitions $\psi_w$ are all continuous.
...
Let $K$ be any compact metric space and consider the Polish space $X=K\sqcup C(K)$, i.e., the disjoint union of $C(K)$ (with its supremum norm topology) and $K$.
...
Observe that all points $g_a^1$ are isolated and that open neighborhoods of $g_{a_0}^0$ are of the form $\{g_a^i:a\in U,i\in\{0,1\}\}\backslash\{g_a^1:a\in F\}$ where $U\subseteq K$ is an open neighborhood of $a_0$ and $F\subseteq K$ is a finite set.
...
The \emph{raison d'être} of the Baire class 1 functions is to have a class of functions that are obtained as equential limit points of continuous functions.
...
There are two natural $\sigma$-algebras one can consider in the product space $\mathbb{R}^\mathcal{P}$: the Borel $\sigma$-algebra, i.e., the $\sigma$-algebra generated by open sets in $\mathbb{R}^\mathcal{P}$, and the cylinder $\sigma$-algebra (i.e., the $\sigma$-algebra generated by the sub-basic open sets in $\mathbb{R}^\mathcal{P}$, that is, the sets $\pi_P^{-1}(U)$ with $U\subseteq \mathbb{R}$ open and $P\in\mathcal{P}$).
...
\begin{rem}
Condition (2) of Theorem~\ref{BFT-2F} shows that to study measure-theoretic versions of NIP, we need only consider compact subsets of $X$.
    Now, every Radon measure on a compact space is finite; hence, by proper normalization, it can be treated as a probability measure.
    Therefore, in the context of Monte Carlo measurability, we focus on Radon probability measures rather than general Radon measures.
\end{rem}
...
We expect to extend most of them to $K$-countably determined (Lindelöf $\Sigma$) spaces or  $K$-analytic spaces, which have previously been used in functional analysis --- see e.g., Kakol et al~\cite{Kakol-Kubis-Lopez-Pelicer-Sobota:2025}.
