...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{newton_div102.png}
        \caption{$n=102$ (Even)}
    \end{subfigure}

    \caption{Iterates of the Newton map $N_{\rho}$ for $\rho(z) = z^3-2z+2$. 
    While points in the basins of the three roots (red, green, blue) converge to fixed values, points in the 2-cycle basin oscillate. 
    Comparing (d), (e), and (f) reveals the subsequential convergence: even iterates converge to the attractor at $z=0$ (cyan), while odd iterates converge to $z=1$ (magenta). 
    The boundaries between these regions form the Julia set of $N_{\rho}$.}
    \label{fig:newton}
\end{figure}
...
For fixed $w\in2^{<\mathbb{N}}$, consider the transition
\begin{align*}
 \phi_w:L&\to L\\
  x&\mapsto
  \begin{cases}
    1_{\bullet}, & x \preccurlyeq w;\\
