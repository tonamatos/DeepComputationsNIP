...
Importantly, it uses mathematical forcing and infinite Ramsey theory techniques that are far from trivial set-theoretical tools.
A derivation of the trichotomy using only elementary methods is currently unknown, and would be of great interest.
...
The most important Polish space is the real line~$\mathbb{R}$.
...
and play an important role in functional analysis, measure theory, dynamical systems, descriptive set theory, and model theory.
...
Using this identification to define the order, but retaining the product topology for continuity,we regard the following CCS $(L,\mathcal{P},\Gamma)$, where $\mathcal{P}$ is a singleton set, and so $\mathcal{L}=L\subseteq\mathbb{R}^\mathcal{P}$.
...
Thus the collection of all of these functions (including the identity), call it $\Gamma$, is a subset of $C(2^\mathbb{N})$.
When composing $\phi_u$ and $\phi_w$, the output of the inner function is $0$ or $1$, which we identify with the infinite binary constant sequences $0^\infty$ and $1^\infty$, so that composition is well-defined.
In contrast with Newton's method, the algebraic structure of $\Gamma$ is quite simple: composing two classifiers results in a Boolean logic gate.
...
For any sizer $r_\bullet$ with $r_\bullet\geq 1$, the shard $L[r_\bullet]$ is all of $L$.
It is worth pointing out that if we topologize $L$ as the standard subspace $[0,1]$ of $\mathbb{R}$, these maps are no longer continuous, and the axiom fails.
...
Routine arguments show that $\lim_{n\to\infty}\phi_{w_n'}=f_a^-$.
The special case when $a$ is the constant zero can be treated separately as it is clearly in the closure.

Topologically, the set $\overline\Gamma$ is particularly interesting, as it is hereditarily separable but not metrizable.
...
This follows from the fact that $\overline{\Gamma}$ contains the characteristic functions of the lexicographic half-lines, distinguishing $x<a$ from $x\leq a$.
