...
By Fact~\ref{baire}t, for perfectly normal $X$, a function $f$ is in $B_1(X,Y)$ if and only if $f^{-1}[U]$ is an $F_\sigma$ subset of $X$ for every open $U\subseteq Y$.
This motivates the following definition:
)
...
The following result (which does not assume countability of the number of features) gives an alternative  characterization of the NIP in terms of universal measurability:
...
Condition (2) of Theorem~\label{BFT-2F} shows that to study measure-theoretic versions of NIP, we need only consider compact subsets of $X$.
    ...
    For this reason, in the context of Monte Carlo measurability, we focus on Radon probability measures rather than general Radon measures.
\end{rem}
