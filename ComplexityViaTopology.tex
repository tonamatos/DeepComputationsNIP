...
\begin{proof}
    ...
    \]
    But then, if $a',b'$ are such that $a<a'<b'<b$, for any finite disjoint $E,F\subseteq\mathbb{N}$ we can choose
    \[
...
Note that,  $\mathcal{L}_{\sh}$ is compact, by remark~\ref{R:probability}, in the context of Monte Carlo computability, we restrict our attention to Radon probability measures instead of  general Radon measures.
...
\begin{proof}
    ...
    and write  as an ultralimit  $f=\mathcal{U}\lim_i\gamma_i$ of computations in $\Delta$.
    Define an extension  $\tilde{f}:\mathcal{L}_{\sh}\rightarrow \mathcal{L}_{\sh}$ of $f$ by $\tilde{f}:=\mathcal{U}\lim_i\tilde{\gamma_i}$.
    Since $\Delta$ is $R$-confined, we have $f:L[r_\bullet]\rightarrow L[r_\bullet]$ and hence $\tilde{f}:\mathcal{L}[r_\bullet]\rightarrow \mathcal{L}[r_\bullet]$ for all $r_\bullet\in R$.
    Lastly, note that for all $r_\bullet\in R$ and $P\in\mathcal{P}$ we have $\pi_P \circ \tilde{f}|_{\mathcal{L}[r_\bullet]}\in \overline{\pi_P\circ\tilde\Delta|_{\mathcal{L}[r_\bullet]}}\subseteq M_r(\mathcal{L}[r_\bullet])$.
\end{proof}
