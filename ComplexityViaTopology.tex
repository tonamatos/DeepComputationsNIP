...
Applying the homeomorphism $f\mapsto \hat{f}$ and using Proposition~\ref{prop:Baire-identifications}, we get $\overline{\hat{A}|_{\mathcal{P}\times K}}\subseteq B_1(\mathcal{P}\times K)$.
...
By Proposition~\ref{prop:Baire-identifications}, it suffices to show that $\pi_P\circ f\in B_1(K)$ for all $P\in\mathcal{P}$.
...
The traditional setting of discrete first-order logic may be regarded as the special case in which predicates are binary (they may take only the values $0$ and $1$, say).
In such setting, the space of types of a structure is compact;
for some purposes, it serves as a sort of compactification of the structure.
...
To bypass this obstacle, we follow the idea introduced in~\cite{alva2024approximability} of covering $\mathcal{L}$ by ``thin'' compact subspaces called \emph{shards}, whose introduction requires some preliminary definitions.

\begin{defn}
A \emph{sizer} is a tuple $r_{\bullet}=(r_P)_{P\in\mathcal{P}}$ of positive real numbers, indexed by~$\mathcal{P}$.
Given a sizer $r_\bullet$, the $r_\bullet$-\emph{shard} of a states structure~$L$ is
\[
...
We define $\mathcal{L}_{\text{sh}}$, as the union of all type-shards (as the sizer $r_{\bullet}$ varies).
\end{defn}
...
If $R$ is a collection of sizers, a subset $\Delta\subseteq\Gamma$ is \emph{$R$-confined} if each $\gamma\in \Delta$ restricts to a map $\gamma|_{L[r_\bullet]}:L[r_\bullet]\to L[r_\bullet]$ (into $L[r_{\bullet}]$ itself) for every $r_\bullet\in R$.
