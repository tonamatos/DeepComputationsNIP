...
The \emph{raison d'être} of the Baire class-1 functions is have a class of functions that contains the continuous functions and is closed under pointwise limits.
...
\]
where $\mu^*$ denotes the outer measure (we need to work with outer measure since the sets $D_k(A,E,a,b)$ need not be $\mu$-measurable).
The inequality certainly holds when $A$ is a countable set of continuous (or $\mu$-measurable) functions.

The main result of this section is that accumulation points of standard computations  (i.e., deep computations) are Monte Carlo computable; this is Theorem~\ref{T:Talagrand stability and Monte Carlo} below.
We now prove that deep computations that are Talagrand $\mu$-stable are $\mu$-measurable.
...
The following technical lemma will be instrumental to prove of Proposition~\ref{prop:Talagrand implies NIP}, which, in turn, will yield the main result of the subsection, Theorem~\ref{T:Talagrand stability and Monte Carlo}.
