...
The Baire Category Theorem states that compact Hausdorff spaces and all completely metrizable space (hence, all Polish spaces) are Baire.%
...
Since both the index set $\mathbb{N}$ is well-ordered, and the value space~$2 = \{0, 1\}$ is also ordered (in the usual way), the states space $2^{\mathbb{N}}$ is lexicographically ordered, say by~$\preccurlyeq$.

For any fixed length $n\in \mathbb{N}$, each finite binary string $y\in 2^n$ will be implicitly identified with its zero-extension to a sequence $\iota(y) = (x_i)_{i\in \mathbb{N}} \in 2^{\mathbb{N}}$, given by
\begin{equation*}
  x_i
  =
  \begin{cases}
...
The “inclusion map” $\iota$ above has a natural left inverse, namely the restriction map
\begin{equation*}
  \cdot\restriction n: 2^{\mathbb{N}}\to 2^n: x\mapsto x\restriction n\coloneqq(x_i)_{i<n}.
\end{equation*}

For fixed $w\in2^{<\mathbb{N}}$, consider the transition $\phi_w:L\to L$ given by the rule
\[
  \phi_w(x)
  =
  \begin{cases}
    1_{\bullet} & (x|_{|w|}\leq_{\mathrm{lex}});\\
...
The main topological tool is Todorčević's remarkable trichotomy for Rosenthal's compacta~\cite{Todorcevic_1999_CompactSubsetsBaire}. 
Our inspiration for this was the work of Glasner and Megrelishvili on tame semigropus in topological dynamics~\cite{glasner2022tame}.
...
Since our interest is in applications to  the theory of computation, we focus our attention on compositional computational structures $(L,\mathcal{P},\Gamma)$  (Definition~\ref{D:CCS}) where the set $\mathcal{P}$ of predicates is countable,
and in sets  computation $\Delta\subseteq\Gamma$ of comptations that are countable.



The following theorem is the cornerstone of the section, as it establishes a powerful  link between deep computations and Rosenthal compacta.
...
Informally,  Theorem~\ref{nip-baire1def} states that if  $\mathcal{P}$ and $\Delta\subseteq\Gamma$ are countable, 
...
Furthermore, Theorem~\ref{nip-baire1def}  shows that every deep computation generated by $\Delta$ is a sequential limit (rather than an abstract ultrafilter limit) of computations in $\Delta$.


\begin{thm}\label{nip-baire1def}
    Let $(L,\mathcal{P},\Gamma)$ be a compositional computational structure (Definition~\ref{D:CCS}) satisfying the Extendibility Axiom (see Definition~\ref{D:extendibility axiom}) with $\mathcal{P}$ countable.
...
The assertion that every deep computation is a pointwise limit of a countable sequence of computations follows from the fact that for a Polish space $X$,s every compact subset of $B_1(X)$ is Fréchet-Urysohn 
...
\label{S:trichotomy}
%By Theorem \ref{nip-baire1def}, if $\Delta$ is a countable set of computations satisfying the NIP on features and shards, 
...
This space was obtained as the closure of the space discussed in Section~\ref{subsec:finite_prec_threshold}, thus giving an example that separates NIP$_2$ from NIP$_3$.
...
\begin{enumerate}
    ...
    Hence, $\Delta$  is NIP$_1$ but not NIP$_2$.
