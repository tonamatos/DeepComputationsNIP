...
\end{equation*}
On the other hand, any rational map extends continuously to an analytic (meromorphic) function on the extended complex plane, so $N_p$ naturally defines a map $\hatC \to \hatC$.

Successive iterations of the Newton-Raphson method (i.e., compositional powers~$N_p^{(n)}$) may be regarded as computations of a suitable CCS whose states space is perhaps~$L_p\subseteq\mathbb{C}$, or perhaps the full extended complex plane~$\hatC$.
...
Per the discusion above, we choose to discuss the Newton-Raphson method as a CCS whose states space is the extended complex plane~$\hatC$.

Let $p = p(z)$ be a nonconstant complex polynomial.
We say that $(\hatC,\mathcal{P},\Gamma_p)$ is \emph{Newton's method for~$p(z)$} if:
\begin{itemize}
...
It is well known —if perhaps surprising at first— that Newton's method fails to converge to any root in many cases.
For example, consider the polynomial $p(z) = z^3-2z+2$ with Newton map
\begin{equation*}
  N_p(z) = z - \frac{z^3-2z+2}{3z^2-2} = \frac{2z^3-2}{3z^2-2}.
\end{equation*}
Successive Newton steps obtained from the initial guess $z_0=0$ yield the divergent sequence $(N_p^{(\bullet)}(0)) = (0, 1, 0, 1, \dots)$ having sequential limits $0, 1$ —neither a root of~$p$.
...
\footnote{While $\infty$ is a fixed point of~$N_p$, it is a repeller:
  if $|z|$ is large, then $N_p(z) = \frac{2}{3}z + O(1)$ has roughly two-thirds its magnitude.
  Therefore, $\infty$ is not the limit of any subsequence of $N_p^{(\bullet)}(z_0)$ with $z_0$ in the Fatou set of~$N_p$.
In particular, $\infty$ is a point of the Newton fractal.}

Recall that the \emph{Fatou set $\mathcal{F}_q$} of a rational map~$q$ on~$\hatC$ consists of all points that have a compact neighborhood on which some subsequence of the sequence $q^{(\bullet)}$ of iterates of~$q$ converges uniformly;
the \emph{Julia set} $\mathcal{J}_q\coloneqq\hatC\setminus \mathcal{F}_q$ is its complement:
it is closed and typically a fractal.
The Julia set of the Newton map~$N_q$ of some rational~$q$ is called the \emph{Newton fractal} of~$q$.
...
For one thing, for many $z$ in the Fatou set, the (entire) sequence $(z_n)$ converges to one of the three roots;
...
The Fatou set also contains points~$z_0$ for which some subsequence of $N_p^{(\bullet)}(z_n)$ converges uniformly to $0$ or~$1$ in some compact neighborhood of~$z_0$;
those points form a region in dark teal color.
\begin{figure}[htbp]
    \centering
