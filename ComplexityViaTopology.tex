...
Given lengths $n, m\in \mathbb{N}$, we have a concatenation operation%
\footnote{When context is clear, the concatenation $v\frown w$ may be denoted simply $vw$.}
$2^m\times 2^n\to 2^{m+n}: (v,w)\mapsto v\frown w$;
such concatenations altogether yield a semigroup structure on~$2^{\mathbb{N}}$ —indeed, a monoid with the empty word $()\in 2^0$ as identity.
The more general concatenation operation $2^{<\mathbb{N}}\times 2^{\mathbb{N}}\to 2^{\mathbb{N}}: (w,x)\mapsto w\frown x$ is a continuous action of $2^{<\mathbb{N}}$ on~$2^{\mathbb{N}}$.
...
Since the index set $\mathbb{N}$ and the value space~$2 = \{0, 1\}$ are well-ordered, the spaces $2^{\mathbb{N}}$ (as well as $2^{<\mathbb{N}}$ and $2^n$ for $n\in \mathbb{N}$) carry a natural lexicographical order henceforth denoted~$\preccurlyeq$;
the order topology coincides with the (product) topology of~$2^{\mathbb{N}}$.
The action of $2^{<\mathbb{N}}$ on~$2^{\mathbb{N}}$ is order-preserving (and continuous), as are all restriction maps~$\cdot\restriction_n$.
...
For any fixed word~$w\in2^n$, let $\phi_w\coloneqq\Brack{\cdot\restriction_n \preccurlyeq w}$ be the relation $x\mapsto \Brack{x\restriction_n\preccurlyeq w}$ on~$2^{\mathbb{N}}$.
% \begin{align*}
%  \overline{\phi}_w: 2^{\mathbb{N}}&\to 2^{\mathbb{N}}\\
%   x&\mapsto
%   \begin{cases}
%     1_{\bullet}, & x\restriction_n \preccurlyeq w;\\
%     0_{\bullet}, & \text{otherwise.}
%   \end{cases}
% \end{align*}
% Such $\phi_w$ are called \emph{finite-precision classifiers}.
Since both $\Brack{\cdot\preccurlyeq w}$ and $\cdot\restriction_n$ are continuous, so is $\phi_w$, which will be called a \emph{finite-precision threshold classifier}.
For $n=0$, there is only one “zeroth-precision” classifier $\phi_{()}$:
the identically true relation $\mathbf{1}$ on~$2^{\mathbb{N}}$;
although insubstantial for our purposes, the subsequent discussion is simpler if we regard (as we shall) the identically false relation $\mathbf{0}$ as finite-precision threshold classifier.
A moment’s reflection shows that any such classifier can be expressed as a Boolean combination%
...
(i.e., $\phi_w(x)$ may be obtained from entries $x_i$ of $x\restriction_n$ using the operations of maximum, minimum and negation $\neg: r\mapsto 1-r$ in place of classical Boolean connectives).%
...
% Let $(w_n)_n\subseteq 2^{<\mathbb{N}}$ be any sequence, and $\coloneqq(\phi_{w_n})_n$
Let $\phi_{\bullet} = (\phi_n)_n\subseteq C_p(2^{\mathbb{N}}, \{0,1\})\subseteq C_p(2^{\mathbb{N}})$ be any sequence of finite-precision threshold classifiers.
Since each $\phi_n$ is non-increasing function on~$2^{\mathbb{N}}$ for all~$n$, the same holds for any accumulation point $\varphi$ of $\phi_{\bullet}$.
...
% \footnote{The identically true relation $\mathbf{1} = \phi_{()}$ is a general classifier.
...
\begin{itemize}
\item $\overline{\phi}_z \coloneqq \Brack{\cdot\preccurlyeq z}$, or
\item $\widehat{\phi}_z \coloneqq \Brack{\cdot\prec z}$
\end{itemize}
for some $z\in 2^{\mathbb{N}}$.
(Note that $\mathbf{0} = \widehat{\phi}_{0_{\bullet}}$, where $0_{\bullet}$ is the constant-zero element of~$2^{\mathbb{N}}$.)
Note that the finite-precision classifiers $\phi_w$ corresponding to words $w\in 2^n$ are general classifiers of the (first) general form:
$\phi_w = \overline{\phi}_{\vec{w}}$
—where the sequence $\vec{w}\coloneqq w\frown 1_{\bullet}$ extends $w$ by values $\vec{w}_i=1$ for $i\ge n$.
Since (countably many) sequences of the form $\vec{w}$ for words $w\in 2^{<\mathbb{N}}$ are dense in~$2^{\mathbb{N}}$, one sees that general classifiers are \emph{sequential} limits of finite classifiers~$\phi_w$, i.e., Baire-$1$ functions on~$2^{\mathbb{N}}$.

A general threshold classifier \emph{not} of the form $\phi_w$ ($=\overline{\phi}_{\vec{w}}$) for any finite word~$w$ is said to be of \emph{infinite-precision};
such infinite-precision classifiers are \emph{discontinuous} Baire-1 functions on~$2^{\mathbb{N}}$:
the sole common point of discontinuity of an infinite-precision classifier $\overline{\phi}_w$ or $\widehat{\phi}_w$ is~$w$.

Let $\Delta = \{\phi_w:w\in 2^{<\mathbb{N}}\} \cup \{\mathbf{0}, \mathbf{1}\}$, where $\mathbf{0},\mathbf{1}: 2^{\mathbb{N}}\rightarrow 2^{\mathbb{N}}$ are the constant maps $x\mapsto0_{\bullet}$ and $x\mapsto1_{\bullet}$, respectively. 
