...
For any fixed word~$w\in2^n$, let $\overline{\phi}_w\coloneqq\Brack{\cdot\restriction_n \preccurlyeq w}$ (resp., $\widehat{\phi}_w\coloneqq\Brack{\cdot\restriction_n \prec w}$) be the relation $x\mapsto \Brack{x\restriction_n\preccurlyeq w}$ (resp., $x\mapsto \Brack{x\restriction_n\prec w}$) on~$2^{\mathbb{N}}$.
% \begin{align*}
...
%   \end{cases}
% \end{align*}
% Such $\overline{\phi}_w$ are called \emph{finite-precision classifiers}.
Clearly, $\cdot\restriction_n$, $\Brack{\cdot\preccurlyeq w}$ and $\Brack{\cdot\prec w}$ (for a \emph{finite} word~$w$) are continuous, so $\overline{\phi}_w$ and $\widehat{\phi}_w$ are also continuous;
they will be called \emph{finite-precision threshold classifiers}.
For $n=0$, such “zeroth-precision” classifiers are the relations $\overline{\phi}_{()} = \mathbf{1}$ (identically true) and $\widehat{\phi}_{()}$ (identically false).
...
Let $(\phi_n)_n\subseteq C_p(2^{\mathbb{N}}, \{0,1\})\subseteq C_p(2^{\mathbb{N}})$ be any sequence of finite-precision threshold classifiers.
Since each $\phi_n$ is non-increasing function on~$2^{\mathbb{N}}$, which is well-ordered by~$\preccurlyeq$, it is easy to see that any accumulation point of $(\phi_n)_n$ is a relation having one of the following forms:%
\footnote{The identically true relation $\mathbf{1} = \phi_{()}$ is a general classifier, but the identically false relation $\mathbf{0} = \widehat{\phi}_{0_{\bullet}}$ is not.
  \label{fn:constant-relations}}
\begin{itemize}
\item $\overline{\phi}_z \coloneqq \Brack{\cdot\preccurlyeq z}$ for some $z\in 2^{\mathbb{N}}$, or
\item $\widehat{\phi}_z \coloneqq \Brack{\cdot\prec z}$ for some $z\in 2^{\mathbb{N}}$, $z\ne 0_{\bullet}$
  (where $0_{\bullet}$ is the constant zero element of~$2^{\mathbb{N}}$).
\end{itemize}
Such  will be called \emph{general threshold classifiers}.
Note that the finite-precision classifiers $\phi_w$ corresponding to words $w\in 2^n$ are still of the (first) general form:
$\phi_w = \overline{\phi}_{\vec{w}}$
—where the sequence $\vec{w}\coloneqq w\frown 1_{\bullet}$ extends $w$ by values $\vec{w}_i=1$ for $i\ge n$.
Using the well-ordering of $2^{\mathbb{N}}$ and the obvious fact that the (countably many) sequences of the form $\vec{w}$ for words $w\in 2^{<\mathbb{N}}$ is dense, it is easy to show that general classifiers $\varphi$ are Baire-$1$ —specifically, sequential limits of finite classifiers $\phi_w$.
