...
For every pointwise bounded  $A\subseteq C_p(X)$, the following properties are all equivalent:
    ...
    \end{enumerate}
  \end{thm}
  (The equivalence \emph{(ii)}$\Leftrightarrow$\emph{(iv)} hold for arbitrary compact~$X$. The equivalence \emph{(ii)}$\Leftrightarrow$\emph{(iii)} hold for arbitrary~$X$.)
\begin{proof}
...
%As we combine ideas of model theory~\cite{Keisler:2023} and topology~\cite{BFT_1978_PCompactBaire}, we are interested in families of real-valued functions that are pointwise bounded. 
Since machines have finite memory, the set of inputs and outputs of any individual computation must be bounded; 
however, the bounds for a particular feature of a computation may depend on the feature.
This leads us to the concepts of \emph{sizer} and \emph{shard} first introduced in~\cite{alva2024approximability}:
...
For every $\varepsilon>0$ and every compact $K\subseteq L$,  there 
...
\[
| \varphi_{Q,\,K,\varepsilon}(v)- Q{\circ}f(v)|\le \varepsilon,
\qquad
\text{for all $v\in K$}.
\]
