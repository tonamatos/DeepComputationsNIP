...
\begin{exmp}\label{E: computation of square roots}
    \textbf{Computation of square roots}. 
        ...
        \begin{equation*}
          ...
          \end{cases}
        \end{equation*}
        As before, $(L,\mathcal{P},\Gamma)$ is a CCS with $L \coloneqq \RP^1\setminus\{0,\infty\} = \mathbb{R}\setminus\{0\}$.
...
Given a finite binary string $w$, we consider the transition $\phi_w:L\to L$ given by the rule \begin{equation*}
	\phi_w(x)=\begin{cases}1^\infty, & \text{if }x|_{|w|}\leq_{\mathrm{lex}}w;\\0^\infty, & \text{otherwise},\end{cases}
\end{equation*} 
...
The following are equivalent.
    ...
    \begin{enumerate}[(i)]
        ...
        \item $\pi_P\circ \Delta|_{L[r_\bullet]}$ satisfies the NIP for all $P\in\mathcal{P}$ and $r_\bullet\in R$; that is, for all $P\in\mathcal{P}$, $r_\bullet\in R$, $a<b$, $\{\gamma_n\}_{n\in\mathbb{N}}\subseteq\Delta$ there are finite disjoint $E,F\subseteq\mathbb{N}$ such that \begin{equation*}
