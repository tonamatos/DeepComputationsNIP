...
\begin{align*}
 \phi_w: 2^{\mathbb{N}}&\to 2^{\mathbb{N}}\\
  ...
  \end{cases}
\end{align*}
Since $(-\infty, w] \coloneqq \{u\in 2^n: u\preceq w\}\subseteq 2^n$ is clopen (since $2^n$ is a finite and Hausdorff —hence discrete— topological space) and $\cdot\restriction_n: 2^{\mathbb{N}}\to 2^n$ is continuous, we see that both $\phi_w^{-1}(1_{\bullet}) = \bigl\{x\in 2^{\mathbb{N}}: x\restriction_n\in (-\infty, w]\bigr\}$ and $\phi_w^{-1}(0_{\bullet}) = 2^{\mathbb{N}}\setminus \phi_w^{-1}(1_{\bullet})$ are clopen, so $\phi_w$ is continuous.

Since the string $w$ is finite, we call $\phi_w$ a \emph{finite-precision classifier}.

Let $\Delta = \{\phi_w:w\in 2^{<\mathbb{N}}\} \cup \{\mathbf{0}, \mathbf{1}\}$, where $\mathbf{0},\mathbf{1}: 2^{\mathbb{N}}\rightarrow 2^{\mathbb{N}}$ are the constant maps with value $0_{\bullet}$ and $1_{\bullet}$, respectively. 
