...
\subsubsection{Newton maps}
\label{sec:Newton-map}

Let $p = p(z)$ be a non-constant rational map with complex coefficients.
...
Any rational map extends continuously to an analytic (meromorphic) function on the extended complex plane, so $N_p$ naturally defines a map $\hatC \to \hatC$;
therefore, successive iterations of the Newton-Raphson method (i.e., compositional powers~$N_p^{(n)}$) may be regarded as computations of a CCS whose states space is $L = \hatC$.
The natural choice of predicates is $\mathcal{P} = (P_1, P_2, P_3)$,
the inverse stereographic projection coordinates.
(The real- and imaginary part maps $z\mapsto\Re{z}$, $z\mapsto\Im{Y}$ are not defined at~$z = \infty$.)
...
\subsubsection{Newton CCSs}
\label{sec:Newton-CCS}
Let $p = p(z)$ be a nonconstant rational map $\hatC\to\hatC$.
We say that the CCS $(\hatC, \mathcal{P}, \Gamma_p)$ is \emph{Newton’s method for~$p(z)$} (or the \emph{Newton CCS for~$p(z)$}) if:
\begin{itemize}
\item $\mathcal{P}:=(P_1,P_2,P_3)$ are the coordinates of inverse steregraphic projection (as above);
% \item $L_p\subseteq \hatC$ is the set of all extended complex numbers $z$ having some neighborhood~$U\subseteq \hatC$ such that the sequence $(N_p^{(n)})_{n\in \mathbb{N}}$ of iterates of the corresponding \emph{Newton map}%
%   \footnote{Implicitly regarded as defined as continuous at $\infty\in\hatC$.}
%   \begin{equation*}
%     N_p:z \mapsto z-\frac{p(z)}{p'(z)}
%   \end{equation*}
...
\item $\Gamma_p:=\{N_p^{(n)}:n\in\mathbb{N}\}$ consists of iterates of the Newton map $N_p$ in equation~\eqref{eq:Np}.
\end{itemize}
The states space $\hatC$ is compact and the Newton map $N_p$ is continuous thereon, so the Extendibility Axiom is satified by Newton CCSs.
...
\paragraph{A real cubic without rational roots}
It is well known —if perhaps surprising at first— that Newton’s method fails to converge to any root%
\footnote{Even when~$z = \infty$ happens to be a root of a rational map~$p(z)$.}
in many cases.
Consider, as a first example, the polynomial ${\rho}(z) = z^3-2z+2$ with Newton map
\begin{equation*}
...
\subparagraph{Visualizing Newton iterates}
...
Recall that the \emph{Fatou set $\mathcal{F}_q$} of a rational map~$q = q(z)$ on~$\hatC$ consists of all points that have a compact neighborhood on which some subsequence of the sequence $q^{(\bullet)}$ of iterates of~$q$ converges uniformly;
the \emph{Julia set} $\mathcal{J}_q\coloneqq\hatC\setminus \mathcal{F}_q$ is its complement:
it is (necessarily closed and) typically a fractal.
The Julia set of the Newton map~$N_q$ is called the \emph{Newton fractal} of~$q$.
...
The deep computations of 

\begin{figure}[htbp]
  \label{fig:z3minus2zplus2}
  \centering
...
\paragraph{The Newton fractal of the $3$-cyclotomic polynomial}
Consider next the $3$-cyclotomic polynomial ${\kappa}(z) = z^3-1$. 
The roots of ${\kappa}(z)$ are the cube roots of unity $r_0=1$ and $r_{\pm1} = \exp(\pm2\pi i/3)$;
the Newton map is
\begin{equation*}
  N_{\kappa}: z\mapsto z-\frac{z^3-1}{3z^2}=\frac{2z^3+1}{3z^2}.
\end{equation*}
In Figure~2(\textsc{C}), one sees a Fatou set consisting of solid-color (disconnected) open sets, one in each of the colors red (for $r_0=1$), green and blue (for $r_{\pm1}$, respectively), where the sequence of all iterates $z_n \coloneqq N_{\kappa}^{(n)}(z_0)$ starting with any point $z_0$ converges to the corresponding root.
...
\begin{exmp}
    \textbf{Newton’s method for $\rho(z) = z^3-2z+2$}.
        Let $B_i$ be the basin of the root $r_i$ ($i = 0, \pm1$), and let $B_{01}$ be the basin of the attractive cycle 0, 1, 0, 1, \dots.
        It may be shown that the Fatou set of $N_\rho$ is $F_\rho = B_{01} \cup B_0 \cup B_{+1} \cup B_{-1}$, each of these four sets being nonempty open.
        Since $B_{01}$ is nonempty, $N_\rho^n$ does not converge pointwise;
        however, the subsequences $N_\rho^{2n}$ and $N_\rho^{2n+1}$ are pointwise convergent to two distinct deep computations $f_0$ (constant $0$ on~$B_{01}$) and $f_1$ (constant $1$ on~$B_{01}$).%
        \footnote{As remarked above, $\infty$ is a repelling fixed point of $N_\rho$, hence $f_0(\infty) = \infty = f_1(\infty)$.}
        The behavior of the iterates $N_\rho$ on the Newton fractal of~$\rho$ (the Julia set $\mathbb{C}\setminus L_\rho$) is chaotic.
\end{exmp}

\subsection{Finite-precision threshold classifiers}\label{subsec:finite_prec_threshold}

Consider the states space $L = 2^\mathbb{N}$ ($= \{0,1\}^{\mathbb{N}}$) consisting of all countable binary sequences $x = (x_n)_{n\in \mathbb{N}} \subseteq \{0,1\}$, endowed with the topology of pointwise convergence.
Let $\mathcal{P} = \{P_n:n\in\mathbb{N}\}$ be the collection of projections (coordinates) $P_n:x \mapsto x_n$.
%In other words, $P_n$ is the predicate that reads the $n$th digit of an input sequence.
Since $2^{\mathbb{N}} \subseteq \mathbb{R}^{\mathbb{N}}$ ($\cong \mathbb{R}^{\mathcal{P}}$) is closed, the type space of $(2^{\mathbb{N}},\mathcal{P})$ is $\mathcal{L} = 2^{\mathbb{N}}$ ($= L$).

Since the index set $\mathbb{N}$ and the value space~$2 = \{0, 1\}$ are well-ordered, the states space $2^{\mathbb{N}}$ is lexicographically ordered, say by~$\preccurlyeq$.

The set $2^n$ of finite binary strings $w$ (“words”) of any fixed length $|w|\coloneqq n$ will be implicitly identified with a subset of~$2^{\mathbb{N}}$ by identifying $w$ with its zero extension $\iota(w) = (x_i)_{i\in \mathbb{N}} \in 2^{\mathbb{N}}$, namely
\begin{equation*}
  x_i
  =
  \begin{cases}
  ...
  \end{cases}  
\end{equation*}
The set of all finite binary strings is thus identified with the subset $2^{<\mathbb{N}} \coloneqq \bigcup_{n\in \mathbb{N}}2^n \subseteq 2^{\mathbb{N}}$.
The “inclusion map” $\iota$ above has a natural left inverse:
the restriction map
\begin{align*}
  \cdot\restriction_n: 2^{\mathbb{N}}&\to 2^n\\
  x&\mapsto x\restriction_n\coloneqq(x_i)_{i<n}.
\end{align*}

Denote by $0_{\bullet}$ (resp., $1_{\bullet}$) the constant sequence with value~$0$ (resp., value~$1$).
For any fixed word~$w\in2^n$, consider the transition
\begin{align*}
...
Since $\{u\in 2^n: u\preceq w\}\subseteq 2^n$ is clopen%
\footnote{$2^n$ is a finite and Hausdorff —hence discrete— topological space.}
and $\cdot\restriction_n: 2^{\mathbb{N}}\to 2^n$ is evidently continuous, it follows that both $\phi_w^{-1}(1_{\bullet}) = \bigl\{x\in 2^{\mathbb{N}}: x\restriction_n\preceq w\bigr\}$ and $\phi_w^{-1}(0_{\bullet}) = 2^{\mathbb{N}}\setminus \phi_w^{-1}(1_{\bullet})$ are clopen, so $\phi_w$ is continuous.
...
Indeed, $\overline{P_n\circ \Delta}$ contains the \emph{Split Cantor} space for all $n\in\mathbb{N}$. (see Example~\ref{E:Rosenthal compacta}-(3)).
...
% In subsection~\ref{S:trichotomy}, we study the implications of Todorčević’s  trichotomy in the realm of deep computations.
% Then, in subsection~\ref{S:heptachotomy}, we focus on the  more recent heptachotomy for separable Rosenthal compacta proved by Argyros, Dodos and Kanellopoulos~\cite{argyros2008rosenthal}.
% Since our interest is in applications to  the theory of computation, we focus our attention on compositional computation structures $(L,\mathcal{P},\Gamma)$  (Definition~\ref{D:CCS}) where the set $\mathcal{P}$ of predicates is countable,
...
\begin{thm}\label{thm:nip-baire1def}
    Let $(L,\mathcal{P},\Gamma)$ be a compositional computation structure (Definition~\ref{D:CCS}) satisfying the Extendibility Axiom (see Definition~\ref{D:extendibility axiom}) with $\mathcal{P}$ countable.
...
%then for each fixed sizer $r_\bullet$  the set $\overline{\tilde\Delta_{\mathcal{L}[r_\bullet]}}$ is a separable Rosenthal compactum (see Definition~\ref{D:Rosenthal compacta}). 
...
Todorčević’s Trichotomy suggests that in order to distinguish the classes $\NIP_i$, the examples in \ref{E:Rosenthal compacta} are essential.
...
The main results of the section are Theorem~\ref{T:NIP and Monte Carlo}  (which connects NIP and Monte Carlo computability) and~\ref{T:Talagrand stability and Monte Carlo}  (which connects Talagrand stability and Monte Carlo computability).
...
This is analogous to Baire class 1 setting (see Proposition~\ref{prop:Baire-identifications}).

We saw in section \ref{sec:prelim}  that, as long as we work with countably many features, PAC-learning (or NIP) corresponds to relative compactness in the space of Baire class 1 functions (see Theorems~\ref{BFT}, \ref{BFT2}, \ref{Generalized BFT}).
...
Note that,  since $\mathcal{L}_{\sh}$ is compact, by remark~\ref{R:probability}, in the context of Monte Carlo computability, we restrict our attention to Radon probability measures instead of  general Radon measures.
...
\begin{proof}
    ...
    Since $\pi_P\circ\Delta|_{L[r_\bullet]}$ has the NIP, so does $\pi_P\circ\tilde\Delta|_{L[r_\bullet]}=\pi_P\circ\Delta|_{\mathcal{L}[r_\bullet]}$,  by Lemma \ref{NIP and closure}.
    Let $f\in\overline{\Delta}$ be a deep computation;
     write $f$ as an ultralimit  $f=\mathcal{U}\lim_i\gamma_i$ of computations in $\Delta$, and
    ...
    Since, by hypothesis, $\Delta$ is $R$-confined, we have $f:L[r_\bullet]\rightarrow L[r_\bullet]$ and hence, $\tilde{f}:\mathcal{L}[r_\bullet]\rightarrow \mathcal{L}[r_\bullet]$.
     Now, by Theorem \ref{BFT-2F}, we have $\overline{\pi_P\circ\tilde\Delta|_{\mathcal{L}[r_\bullet]}}\subseteq M_r(\mathcal{L}[r_\bullet])$;
...
Note that the reference to outer measure in Definition~\ref{D:Talagrand stable} is necessary since $D_k(A,E,a,b)$ need not be measurable.
The inequality certainly holds when $A$ is a countable set of continuous (or $\mu$-measurable) functions.

\begin{rems}
\label{R:Talgrand stability}
...
\item
If $\overline{A}$ is stable, then $A$ is $\mu$-stable. (This is immediate from (2).)
\end{enumerate}
\end{rems}

The main result of this section is that deep computations that are limit points of a Talagrand stable set of computations are Monte Carlo computable; this is Theorem~\ref{T:Talagrand stability and Monte Carlo} below.
To prove it, we first show that the closure of a Talagrand $\mu$-stable set are $\mu$-measurable (lemma~\ref{talagrand stable is relatively compact});
...
\begin{proof}
If $A$ is Talagrand $\mu$-stable, so is $\overline{A}$ by Remarks~\ref{R:Talgrand stability}
    ...
    Thus, $\{f\}$ is not $\mu$-stable. However, we saw in Remarks~\ref{R:Talgrand stability} that a subset of a $\mu$-stable set must be $\mu$-stable,
...
Since, for any such $\mu$,  $A$ is Talagrand $\mu$-stable Lemma \ref{talagrand stable is relatively compact} gives us $\overline{A}\subseteq\mathscr{M}^0(X,\mu)$.
...
\begin{proof}
    This follows directly from Proposition \ref{prop:Talagrand implies NIP} and Theorem~\ref{T:NIP and Monte Carlo}.
\end{proof}
