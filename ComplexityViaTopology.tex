...
The embedding of computations into spaces of types then allows us to utilize the vast theory of topology of function spaces, known as \emph{$C_p$-theory}, to obtain results about complexity of topological limits of computations.
...
%By contradiction, let us assume that $A$ is relatively countably compact in~$B_1(X)$, but there exists some $f\in \overline{A}\setminus B_1(X)$.
...
Thus, Theorem~\ref{thm:BFT} becomes (for pointwise bounded function collections $A\subseteq C_p(X)$) that $A$ is relatively compact in $B_1(X)$ if and only if $A$ has BFT-NIP.
...
The field of $C_p$-theory was pioneered by A.~V.~Arhangel’skiĭ and his students in the 1970s and 1980s~\cite{Arkhangelskii:1992}, and
...
Let $\varphi_{\bullet} = (\varphi_n)_n\subseteq C_p(2^{\mathbb{N}}, \{0,1\})\subseteq C_p(2^{\mathbb{N}})$ be any sequence of finite-precision threshold classifiers.
Since each $\varphi_n$ is non-increasing function on~$2^{\mathbb{N}}$ for all~$n$, the same holds for any accumulation point $\varphi$ of $\varphi_{\bullet}$.
It is easily shown that such accumulation points~$\varphi$, henceforth called \emph{ultra-classifiers}, are (unary) relations having, for some sequence $z\in 2^{\mathbb{N}}$, one of the forms%
...
When threshold classifiers are regarded as elements of $B_1(2^{\mathbb{N}})$, we will denote ${\preceq}x$ by $\bar{x}$, and ${\prec}x$ by~$\hat{x}$.
Per the preceding discussion, such classifiers constitute a set
\begin{equation*}
  \cancel{C} = \widehat{C} \cup \overline{C} \subseteq B_1(2^{\mathbb{N}})
\end{equation*}
...
The upshot is that, starting from a countable set of quite simple finite-precision classifiers (nonincreasing boolean functions) on Cantor space, their accumulation points (“ultraclassifiers”, so to speak) constitute a much more complicated split Cantor $\cancel{C}\subseteq B_1(2^{\mathbb{N}})$.
...
\label{sec:value-tests}
% Let $\varphi_{\bullet} = (\varphi_n)_n\subseteq C_p(2^{\mathbb{N}}, \{0,1\})\subseteq C_p(2^{\mathbb{N}})$ be any sequence of prefix tests.
...
The space $\mathcal{A} = \widetilde{\mathcal{A}} \cup \bbar{\mathcal{A}} \cup \{0_{\bullet}\}$ is (homeomorphic to) the \emph{extended Alexandroff compactification of~$2^{\mathbb{N}}$}.
This space possesses key topological property that $\mathbf{0}$ is not a $G_\delta$ point, i.e., $\{\mathbf{0}\}$ is not a countable intersection of open sets;
$\mathbf{0}$ is the only such non-$G_\delta$ point.
It follows that the space of ultra-prefix tests is not first-countable
(in a first-countable Hausdorff space all points are~$G_\delta$). 
The subspace $\bbar{\mathcal{A}}$ is discrete of size continuum.
...
\end{enumerate}
 ...
 there is $\tilde f:\mathcal{L}_{\sh}\rightarrow \mathcal{L}_{\sh}$ such that $\tilde f|_{\mathcal{L}[r_\bullet]}\in B_1(\mathcal{L}[r_\bullet],\mathcal{L}[r_\bullet])$ for all $r_{\bullet}\in R$.
...
Then, for all $r_\bullet\in R$ we have $\tilde f|_{\mathcal{L}[r_\bullet]}\in\overline{\tilde\Delta|_{\mathcal{L}[r_\bullet]}}\subseteq B_1(\mathcal{L}[r_\bullet],\mathcal{L}[r_\bullet])$.
The assertion that every deep computation is a pointwise limit of a countable sequence of computations follows from the fact that for a Polish space $X$, every compact subset of $B_1(X)$ is Fréchet-Urysohn 
...
Hence, this is a Rosenthal compactum that is not hereditarily separable (and therefore not first countable).
In particular, this space is not separable.
However, as the next example shows, it can be made separable space by ``softening'' the neighborhoods of each point through the addition of a countable set.
...
Note that this space is a separable Rosenthal compactum that is not first countable.
This is the example discussed in Section \ref{sec:prefix-test}. 
It is an example of a CCS that is NIP but not $\NIP_1$.

    \item \emph{Split Cantor}.
Let $<$ be the lexicographic order in the space $2^\mathbb{N}$ of infinite binary sequences.
...
This space was obtained as the closure of the space discussed in Section~\ref{sec:Cantor-classifiers}, thus giving an example that separates $\NIP_2$ from $\NIP_3$.

This is a well known separable Rosenthal compactum;
an example of a countable dense subset is the set of all $f_a^+$ and $f_a^-$ where $a$ is an infinite binary sequence that is eventually constant.
...
Let $K$ be any compact metric space, and consider the Polish space $X=K\sqcup C(K)$, i.e., the disjoint union of $K$ and $C(K)$ (the space of real-valued continuous functions on $K$ with the supremum norm topology).
...
As with the Alexandroff compactification $A(2^{\mathbb{N}})$, we can make the space $D(2^{\mathbb{N}})$ separable  by ``softening'' the neighborhoods of each point through the addition of a countable set.
...
\node {$K$ is separable Rosenthal compactum}
  ...
  child {node {$K$ is not metrizable}
    ...
    child {node {\hspace{2cm} $K$ is not hereditarily separable}
      child {node {$K$ is first countable \\ (copy of $D(2^\mathbb{N})$ or $\hat{D}(S(2^\mathbb{N}))$)}}
