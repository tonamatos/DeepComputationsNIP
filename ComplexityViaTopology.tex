...
\subsection{Example: Finite-precision threshold classifiers}\label{subsec:finite_prec_threshold}

Consider the states space $L = 2^\mathbb{N}$ ($= \{0,1\}^{\mathbb{N}}$) consisting of all bit sequences $x = (x_n)_{n\in \mathbb{N}} \subseteq \{0,1\}$.%
\footnote{Endowed with the topology of pointwise convergence, the product space~$2^{\mathbb{N}}$ is compact and homeomorphic to the Cantor (ternary) subset of the interval~$[0,1]$.}
...
The constant $0$ and $1$ elements of $2^n$ are denoted $0_{\bullet}$ and $1_{\bullet}$, respectively.
...
\subsubsection{Concatenation action and prefixes}
\label{sec:concat}
...
for clarity, we may always write ${\preccurlyeq}_nw$ where $n=|w|$.
