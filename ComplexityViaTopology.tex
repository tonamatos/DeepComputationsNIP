...
If the word ``finite'' is omitted in the above definitions, we obtain the definitions of \emph{infinitary} NIP (weaker than finitary NIP) and \emph{infinitary} IP (stronger than finitary IP), respectively.


If we insist on witnesses $E,F\subseteq \mathbf{N}$ such that $E\cup F = \mathbf{N}$, we call the respective properties ``BFT-NIP'' (even weaker than infinitary NIP) and ``BFT-IP'' (even stronger than infinitary IP).
...
Our goal now is to characterize relatively compact subsets of $B_1(X,\mathbb{R}^\mathcal{P})$ for the particular case when $\mathcal{P}$ is countable.
