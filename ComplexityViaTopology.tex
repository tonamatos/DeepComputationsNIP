...
The set $2^n$ of finite binary strings $w$ (“words”) of arbitrary fixed length $|w|\coloneqq n$ will be implicitly identified with a subset of~$2^{\mathbb{N}}$ by identifying $w$ with its zero extension $\iota(w) = (x_i)_{i\in \mathbb{N}} \in 2^{\mathbb{N}}$, namely
\begin{equation*}
  x_i
  =
  \begin{cases}
...
Since $\{u\in 2^n: u\preccurlyeq w\}\subseteq 2^n$ is clopen%
\footnote{$2^n$ is a finite and Hausdorff —hence discrete— topological space.}
and $\cdot\restriction_n: 2^{\mathbb{N}}\to 2^n$ is evidently continuous, it follows that both $\phi_w^{-1}(1_{\bullet}) = \bigl\{x\in 2^{\mathbb{N}}: x\restriction_n\preccurlyeq w\bigr\}$ and $\phi_w^{-1}(0_{\bullet}) = 2^{\mathbb{N}}\setminus \phi_w^{-1}(1_{\bullet}) = \bigl\{x\in 2^{\mathbb{N}}: x\restriction_n\succ w\bigr\}$ are clopen, so $\phi_w$ is continuous.
