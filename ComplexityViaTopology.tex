...
The action of $2^{<\mathbb{N}}$ on~$2^{\mathbb{N}}$ is order-preserving, as are all restriction maps~$\cdot\restriction_n$.
...
(For $n=0$, there is only one “zeroth-precision” classifier $\phi_{()}$ equal to the identically true relation $\mathbf{1}\equiv 1$ on~$2^{\mathbb{N}}$.)
A moment’s reflection shows that any such classifier $\phi_w$ can be expressed as a Boolean combination%
