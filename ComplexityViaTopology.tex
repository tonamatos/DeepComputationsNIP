...
\footnote{We choose square brackets \emph{à la} Knuth with a double-strike “blackboard bold” typesetting convention precluding any ambiguity.}
   Iverson’s bracket by $\Brack{\cdot}$, according to the following convention:
  for any statement $S$, the value of $\Brack{S}$ is $1$ if $S$ is true, $0$ if false.
\end{notn}
...
Clearly, the classifier $\phi_w$ may be expressed as a Boolean combination%
...
of the 2-valued predicates $P_i(w)$ for $i<n$
(i.e., $\phi_w$ may be expressed in terms of the values $P_i(w)$ using the binary operations $\vee$ —maximum— and $\wedge$ —minimum— plus the unary negation operation $\neg: r\mapsto 1-r$ that exchanges the logical values $0$ and~$1$).
