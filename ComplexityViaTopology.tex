...
At the moment of writing this paper, decades after his original argument, no elementary proof has been found~\cite{Todorcevic:2023,Todorcevic:2026}.
...
The final Section~\ref{Measure-theoretic NIP} introduces a probabilistic viewpoint, the development of which we intend to pursue in future research, extending the present framework to encompass non-deterministic and quantum computations.
...
\begin{fact}[Baire]\label{baire}
    ...
    \end{itemize}
    If, mooreover, $X$ is Baire, then (1) and (2) are equivalent to:
    \begin{itemize}
...
\section{Probabilistic versions of NIP and Monte Carlo computability of deep computations}
...
\begin{thm}[Bourgain-Fremlin-Talagrand, Theorem 2F in \cite{BFT_1978_PCompactBaire}]
\label{BFT-2F}
...
\begin{rem}
Condition (2) of Theorem~\label{BFT-2F} shows that to study measure-theoretic versions of NIP, we need only consider compact subsets of $X$.
...
The main result of this section is that accumulation points (deep computations) are Monte Carlo computable (Theorem~\ref{T:Talagrand stability and Monte Carlo}).
We now prove that deep computations that are Talagrand $\mu$-stable are $\mu$-measurable.
But first, we mention the following useful characterization of measurable functions (compare with Fact \ref{baire}):
...
The following lemma will be instrumental to prove of Proposition~\ref{prop:Talagrand implies NIP}, which, in turn, will yield the main result of the subsection, Theorem~\ref{T:Talagrand stability and Monte Carlo}.
...
\begin{proof}
    ...
    Thus, $\{f\}$ is not $\mu$-stable. However,  we argued before that a subset of a $\mu$-stable set must be $\mu$-stable,
...
It turns out that, even in the simple case of countably many computations, this question is sensitive to the set-theoretic axioms. 
