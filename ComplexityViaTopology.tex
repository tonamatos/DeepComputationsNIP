...
It is easily shown that such accumulation points~$\varphi$, henceforth called \emph{general threshold classifiers}, are (unary) relations having, for some sequence $z\in 2^{\mathbb{N}}$, one of the forms%
...
all other finite-precision classifiers are of the form ${\preccurlyeq}_nw$ and hence identical to the general classifier ${\preceq}\arc{w1_{\bullet}}$, as remarked above.
Since (countably many) sequences of the form $\arc{w1_{\bullet}}$ for words $w\in 2^{<\mathbb{N}}$ are dense in~$2^{\mathbb{N}}$, one sees that general classifiers are \emph{sequential} limits of finite classifiers, hence are Baire-$1$ functions on~$2^{\mathbb{N}}$.
