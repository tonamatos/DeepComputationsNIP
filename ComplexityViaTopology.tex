...
\begin{figure}[htbp]
    ...
    \caption{Behavior of iterates of the Newton map for ${\rho}(z) = z^3-2z+2$. 
      Figure~(\textsc{C}) —where the “color fuzzines” of~(\textsc{A}) and~(\textsc{B}) has already visually disappeared— is \emph{de facto} a depiction of the Fatou and Julia sets of~$N_{\rho}$.
      The (dense) Fatou set of~$N_{\rho}$ consists of:
      (\emph{i})~open red, green and blue regions of convergence towards a root $r_0, r_{\pm1}$, and
      (\emph{ii}) another open region (in teal) of points whose sequence of Newton iterates tends to the orbit $\{0, 1\}$.
      the Newton fractal of~${\rho}$ (the Julia set of~$N_{\rho}$) is the closed nowhere dense complement of the Fatou set of~$N_{\rho}$.}
    \label{fig:newton}
\end{figure}
