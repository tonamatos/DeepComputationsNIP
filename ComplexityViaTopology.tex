...
In \cite{alva2024approximability}, we proved a new ``tame vs wild'' (i.e., polynomially approximable vs nonpolynimially approximable) dichotomy for the complexity of deep computations by invoking a classical result of Grothendieck from the 1950s~\cite{Grothendieck:1952}.
...
In the interest of completeness, we some proofs that may be safely skipped by readers familiar with these topics.
...
This can be visualizaed by adding a dash of color:
...
By~\cite[Corollary 4.6]{Blanchard1984}, $L$ is open and dense in $\mathbb{C}$.
...
Computations are the iterations of Newton's method, i.e., $\Gamma=\{N_p^n:n\in\mathbb{N}\}$.
...
Thus, $(L,\mathcal{P},\Gamma)$ satisfies the Extendibility Axiom.
...
\begin{exmp}\label{E: computation of square roots}
    \textbf{Computation of square roots}. 
        ...
        The Newton's method map $N_p:L\rightarrow L$ is given by $$N_p(x)=\frac{x^2+a}{2x}.$$
...
\begin{exmp}
    \textbf{Newton's method for $p(z)=z^3-2z+2$}.
        ...
        Let $L=B\cup\bigcup_{i=1}^3 B_i$ and $J$ be its complement, i.e., the Julia set.
...
Let $\mathcal{P}=\{P_n:n\in\mathbb{N}\}$ is the collection of projections, i.e., $P_n(x)=x(n)$ for each $n$ and each $x\in L$.
...
Indeed, $\overline{P_n\circ \Delta}$ contains the \emph{Split Cantor} space for all $n\in\mathbb{N}$. (see Examples~\ref{E:Rosenthal comacta}(3)).
...
In fact, $\overline{\Delta}=\Delta\cup\{\delta_a:a\in 2^\mathbb{N}\}\cup\{\mathbf{0^\infty}\}$ and this space is known as the \emph{Extended Alexandroff compactification of $2^\mathbb{N}$} (see Example \ref{E:Rosenthal comacta}(2)).
...
Todorčević's Trichotomy suggests that in order to distinguish the classes NIP$_i$, the examples in \ref{E:Rosenthal comacta} are essential.
...
\begin{enumerate}
    ...
    We saw that $\overline{\Delta}$ is homeomorphic to the Split Cantor space $S(2^\mathbb{N})$ (Example \ref{E:Rosenthal comacta}(3)), which is hereditarily separable but not metrizable.
    ...
    We saw that $\overline{\tilde{\Delta}}$ is homeomorphic to the Extended Alexandroff compactification $\hat{A}(2^\mathbb{N})$ (Example \ref{E:Rosenthal comacta}(3)), which is separable but not first countable.
