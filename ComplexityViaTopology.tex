...
\begin{exmp}\label{E: computation of square roots}
    \textbf{Computation of square roots}. 
        ...
        For example,
        $$\tilde{N_p}(x)=\left\{\begin{cases}{cc}
        ...
        Moreover,
        $$f(x)=\left\{\begin{cases}{cc}
            \sqrt{a}, & x>0  \\
            -\sqrt{a} & x<0
        \end{cases}\right.$$
        Notice that $f$ can be extended to $\mathcal{L}$ by
        $$\tilde{f}(x)=\left\{\begin{cases}{cc}
            \sqrt{a}, & x>0  \\
            -\sqrt{a}, & x<0  \\
            \infty, & x=0,\infty
        \end{cases}\right.$$
...
Given a finite binary string $w$, we consider the transition $\phi_w:L\to L$ given by the rule $$\phi_w(x)=\begin{cases}1^\infty, & x|_{|w|}\leq_{\mathrm{lex}}w\\0^\infty, & \text{otherwise}\end{cases},$$ where $|w|$ is the length of the string $w$ and $x|_{|w|}$ is the prefix of $x$ of length $|w|$.
