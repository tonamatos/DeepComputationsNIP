...
The set of words of any finite length is the disjoint union $2^{<\mathbb{N}}\coloneqq \bigsqcup_{n\in \mathbb{N}}2^n$ (a discrete space);
...
such concatenations give a binary operation on~$2^{\mathbb{N}}$.
In case $m\ge n$, we also have natural continuous restriction maps (projections)
...
Restriction maps $\cdot\restriction_n$ are order-preserving.
...
the sole common point of discontinuity of an infinite-precision classifier $\underline{\phi}_w$ or $\widehat{\phi}_w$ is~$w$.
...
We saw in section \ref{sec:prelim}  that, as long as we work with countably many features, PAC-learning (or NIP) corresponds to relative compactness in the space of Baire class 1 functions (see Theorems~\ref{thm:BFT}, \ref{thm:BFT2}, \ref{thm:Generalized-BFT}).
