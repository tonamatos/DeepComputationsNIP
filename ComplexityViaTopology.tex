...
\subsection{States: types and topology}
\label{sec:states-types-topology}
...
We take $L\subseteq \mathbb{R}^{\mathcal{P}}$ as implicitly identified with a subset of the type space~$\mathcal{L} \subseteq \mathbb{R}^{\mathcal{P}}$ of~$(L, \mathcal{P})$.
...
We topologize $\Gamma$ as a subset of the topological product $L^L$, where (as usual) the ``exponent''~$L$ serves merely as an index set, but the ``base''~$L\subseteq \mathbb{R}^{\mathcal{P}}$ is topologized as a subspace.
...
Intuitively, such ultracomputations are “final” or “ultimate”.
...
\begin{rem}
(Related to Remark~\ref{R:types}.)
Compositional computational structures as defined above are not real-valued structures in a literal sense (e.g., as in~\cite{Keisler:2023}).
However, in a semantically transparent manner, any CSS as above may alternatively be seen (as per its original definition in~\cite{alva2024approximability}) as a real-valued structure $(L,\mathcal{P},\Gamma,\circ,\operatorname{ev})$, where
\begin{itemize}
\item
