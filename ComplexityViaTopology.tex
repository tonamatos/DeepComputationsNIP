...
The idea of embedding models of theories into their type spaces is borrowed from model theory, where types spaces play a central role for classification of theories.
...
In a previous paper~\cite{alva2024approximability}, the authors introduced the concept of limits of computations, which we called \emph{ultracomputations} (given they arise as ultrafilter limits of standard computations) and \emph{deep computations} (following usage in machine learning~\cite{Bai:2019}).
...
In this paper, we refine the dichotomy obtained in \cite{alva2024approximability}, and obtain new complexity classes for deep computation. 
Both the topological and model-theoretic tools are considerably more sophisticated.
In particular, the ``wild'' part of the dichotomy from~\cite{alva2024approximability}, is further subdivided between ``tame'' and ``wild''  classes, three new levels of  tameness are identified within this new subdivision, and seven distinct prototypes of tameness are detected.
...
In this paper, we apply the theory of spaces of Baire class~1 functions to identify and separate new complexity classes of deep computations.
We obtain a new ``tame vs wild'' dichotomy for  deep computations
by invoking a famous paper by Bourgain, Fremlin and Talagrand from the late 70s~\cite{BFT_1978_PCompactBaire}, 
and further, a new trichotomy for the class of ``tame'' deep computations by invoking an equally celebrated result of Todorčević, from the late 90s, for functions of the first Baire class~\cite{Todorcevic_1999_CompactSubsetsBaire}.

Todorčević’s trichotomy concerns \emph{Rosenthal compacta}, which are topological spaces that fit our context superbly:
they are defined as compact spaces that are homeomorphic to subspaces of the space of Baire class~1 functions on some Polish (separable, completely metrizable) space, under the topology of pointwise convergence.
...
At the time of writing this paper, decades after his original argument, no elementary proof has been found~\cite{Todorcevic:2023,horowitz2019compactsetsbaireclass}.
...
Refining Todorčević’s trichotomy, we invoke a more recent heptachotomy for separable Rosenthal compacta obtained by Argyros, Dodos and Kanellopoulos~\cite{argyros2008rosenthal};
...
The final section, Section~\ref{Measure-theoretic NIP}, introduces a probabilistic viewpoint, the development of which we intend to pursue in future research, extending the present framework to encompass non-deterministic and quantum computations.
...
The question of whether we can remove the set-theoretic assumption in Theorem \ref{T:nip implies talagrand stability} when $A\subseteq C_p(X)$  (instead of $A\subseteq M_r(X)$) remains open.
