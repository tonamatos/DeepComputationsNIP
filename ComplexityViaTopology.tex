...
For any fixed lengths $n\le m\in \mathbb{N}$, by restriction, we obtain natural continuous maps
$2^{\mathbb{N}}\rightarrow 2^m\rightarrow 2^n$.
The restriction of~$x$ —either a sequence or a word of length $m\ge n$— to a length-$n$ word is denoted $x\restriction_n$ (i.e., $\cdot\restriction_n$ denotes the restriction map onto~$2^n$).
...
\begin{notn}
  When useful, we will regard $0, 1$ as logical constants ($0$ for falsehood, $1$ for truthfulness).
  The propositional connectives of negation~“$\neg$”, disjunction~“$\vee$” and conjunction~“$\wedge$” are realized using simple (lattice) operations on~$\mathbb{R}$;
  explicitly, as $r\mapsto 1-r$ (negation), binary maximum (max) and binary minimum (min), respectively.

  We will denote Iverson’s bracket%
\footnote{We choose square brackets \emph{à la} Knuth with a double-strike “blackboard bold” typesetting convention precluding any ambiguity.}
 by~$\Brack{\cdot}$;
...
For any fixed word~$w\in2^n$, let $\phi_w\coloneqq\Brack{\cdot\restriction_n \preccurlyeq w}$ be the binary (i.e., $\{0,1\}$-valued) function $2^{\mathbb{N}}\rightarrow 2^n: x\mapsto \Brack{x\restriction_n\preccurlyeq w}$.
% \begin{align*}
...
A moment’s reflection shows that the classifier $\phi_w$ may be expressed as a Boolean formula%
...
in the values of the predicate values $P_i(x)=x_i$ (for $i<n$) of its argument~$x\in 2^{\mathbb{N}}$
(i.e., $\phi_w(x)$ may be expressed%
\footnote{The specific expression depends on~$w$.}
in terms of the logical values $x_i\in \{0,1\}$ using the operations of maximum, minimum and negation $\neg: r\mapsto 1-r$ in place of classical Boolean connectives).
...
Since each $\phi_n$ is non-increasing function on~$2^{\mathbb{N}}$, which is well ordered, it is easy to see that any accumulation point $\varphi$ of $(\phi_n)_n$ is of one of the forms, for some $z\in 2^{\mathbb{N}}$:
\begin{itemize}
\item $\overline{\phi}_z \coloneqq \Brack{\cdot\preccurlyeq z}$, or
\item $\widehat{\phi}_z \coloneqq \Brack{\cdot\prec z}$;
\end{itemize}
these will be called \emph{general threshold classifiers}.
Note that the finite-precision classifiers $\phi_w$ above for some word $w\in 2^n$ are still of the (first) form $\phi_w = \overline{\phi}_{\vec{w}}$, where the sequence $\vec{w}$ extends $w$ by zero values $\vec{w}_i=0$ for $i\ge n$.
In fact, using the well-ordering and separability of $2^{\mathbb{N}}$ and the obvious fact that a set of countably many  sequences —those of the form $\vec{w}$ for words $w\in 2^{<\mathbb{N}}$— is dense, it is easy to show that general classifiers $\varphi$ are Baire-1 —indeed, sequential limits of finite classifiers $\phi_w$.

A general threshold classifier \emph{not} of the form $\phi_w = \phi_{\overline{w}}$ for any finite word~$w$ is said to be of \emph{infinite-precision};
