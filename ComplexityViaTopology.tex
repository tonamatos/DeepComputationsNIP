...
\subsection{Newton’s method as a CCS}\label{sec:Newton-method}
...
\paragraph{Newton maps}
\label{par:Newton-map}
...
\paragraph{The cubic  ${\rho}(z) = z^3-2z+2$}
...
\begin{exmp}\label{exa:square-roots-compute}
...
Fundamental in this section is a measure-theoretic version of Theorem \ref{thm:Generalized-BFT}, namely,  Theorem~\ref{BFT-2F}. 
...
\begin{lem}\label{lem:talagrand-stab-relcompct}
...
Since, for any such $\mu$,  $A$ is Talagrand $\mu$-stable, Lemma \ref{lem:talagrand-stab-relcompct} gives us $\overline{A}\subseteq\mathscr{M}^0(X,\mu)$.
